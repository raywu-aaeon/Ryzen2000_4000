...
extern AMI_NETWORK_BLOCKED  BlockedIp6BootNetworkList[];
#endif
  ...
  )
{
  ...
  HiiGetBrowserData (&gIp6ConfigNvDataGuid, mIp6ConfigStorageName, BufferSize, (UINT8 *) IfrNvData);
#else
  ...
  if (!EFI_ERROR (Status)) {
    ...
    BufferSize = sizeof (IP6_CONFIG_IFR_NVDATA);
#if NET_PKG_AMI_PORTING_ENABLE
	HiiSetBrowserData (&gIp6ConfigNvDataGuid,mIp6ConfigStorageName, BufferSize, (UINT8 *) IfrNvData, NULL);
#else
  ...
  )
{
  ...
  Ip6NvData = &Instance->Ip6NvData;
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
  if(!IsAmiNetworkDeviceBlocked(IpSb->Controller, (AMI_NETWORK_BLOCKED*)BlockedIp6BootNetworkList)) {
#endif      
      Ip6FreeAddressInfoList (&Ip6NvData->ManualAddress);
      Ip6FreeAddressInfoList (&Ip6NvData->GatewayAddress);
      Ip6FreeAddressInfoList (&Ip6NvData->DnsAddress);
