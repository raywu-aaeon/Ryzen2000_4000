  ...
  )
{
  ...
  if (DeviceInfo->PowerManagementStatusAndControlRegister != 0x00) {
    if ((PciRead16 (PciLibAddress + DeviceInfo->PowerManagementStatusAndControlRegister) & (BIT0 | BIT1)) != 0x00) {
      ...
      //
//**************AMI PORTING BEGIN   ****************
      SerialPortWriteRegister (SerialRegisterBase, R_UART_IIR_FCR, (UINT8)(PcdGet8 (PcdSerialFifoControl) & (B_UART_FCR_FIFOE | B_UART_FCR_FIFO64)));
//**************AMI PORTING END   ****************
    }
  }
...
//**************AMI PORTING BEGIN   ****************
  UartIirReg = SerialPortReadRegister (SerialRegisterBase, R_UART_IIR_FCR);
  Temp = (PcdGet8 (PcdSerialFifoControl) & BIT0);
  if ((UartIirReg >> 7 ) == Temp){
      if ((Temp == 1) && (((UartIirReg >> 6) & BIT0) != Temp)) {
          Initialized = FALSE;
      }
  } else {
      Initialized = FALSE;
...
//AMI PORTING END
  ...
  //
//**************AMI PORTING BEGIN   ****************
  SerialPortWriteRegister (SerialRegisterBase, R_UART_IIR_FCR, 0x00);
  SerialPortWriteRegister (SerialRegisterBase, R_UART_IIR_FCR, (UINT8)(PcdGet8 (PcdSerialFifoControl) & (B_UART_FCR_FIFOE | B_UART_FCR_FIFO64)));
//**************AMI PORTING END   ****************
