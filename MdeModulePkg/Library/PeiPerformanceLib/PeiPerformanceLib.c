...
//*** AMI PORTING BEGIN ***//
//--- Add function to get module name from FFS UI section
BOOLEAN GetModuleNameFromGuid (IN EFI_GUID *Guid, OUT CHAR8* Str)
{   
    EFI_STATUS                      Status;   
    UINT8                           Index = 0;
    ...
    EFI_FFS_FILE_HEADER             *FilePointer;
    UINTN                           Instance = 0;
    UINTN                           FileNameLength, FileNameLength8;
    ...
    while (TRUE) {
        Status = (*PeiServices)->FfsFindNextVolume (PeiServices, Instance, (VOID**)&Fv);
        if (EFI_ERROR (Status)) {
            break;
        }
        FilePointer = NULL;
        ...
        ///
        while (TRUE) {
            Status = (*(PeiServices))->FfsFindNextFile (PeiServices, EFI_FV_FILETYPE_PEIM, Fv, (VOID**)&FilePointer);
            if (Status == EFI_NOT_FOUND) {
                break;
            }
            if (!CompareMem(&(FilePointer->Name),Guid,sizeof(EFI_GUID))) {    /// FV Found
                FfsLength = FilePointer->Size[0] + (FilePointer->Size[1] << 8) + (FilePointer->Size[2] << 16);
                FilePointer ++;
                TempFfsData = (UINT8*)FilePointer;
                ...
                if (UiFind) {
                    ...
                    FileNameLength8 = (FileNameLength - 4) / 2;
                    if (FileNameLength8 > STRING_SIZE) { /// over length, skip this data.
                        FileNameLength8 = STRING_SIZE;
                    }
                    for (Index = 0; Index < FileNameLength8; Index++) { /// Save token name
                        if (Index == STRING_SIZE) {
                            break;
                        }
                        Str[Index] = (CHAR8) TempFfsPtr[0];
                        TempFfsPtr += 2;
                    }
...
//*** AMI PORTING END *****//
  ...
  case MODULE_END_ID:
//*** AMI PORTING BEGIN ***//
//--- Try to get Module name from FFS UI section using GUID
    if  (GetModuleNameFromGuid ((EFI_GUID*)ModuleGuid, &StrArray[0]))
        StringPtr = &StrArray[0];
    else    
//*** AMI PORTING END *****//                      
        StringPtr = PEIM_TOK;
    if (!PcdGetBool (PcdEdkiiFpdtStringRecordEnableOnly)) {
