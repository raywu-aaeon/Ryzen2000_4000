  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // Bug fix: The condition below should be the same as
  // condition in the PEI Core dispatcher the implements
  // the shadowing logic. It was "BootMode != BOOT_ON_S3_RESUME" at
  // some point, however, evolved since.
  // 
  // The implications of the unfixed bug are:
  // When PcdMigrateTemporaryRamFirmwareVolumes is FALSE, 
  // there is a performance penalty
  // When PcdMigrateTemporaryRamFirmwareVolumes is TRUE,
  // system may crash on S3 resume while accessing un-shadowed
  // PPIs published by DxeIpl (such as section extraction PPI),
  // because some systems are configured to trigger CPU exception
  // on ROM access after memory installation.
