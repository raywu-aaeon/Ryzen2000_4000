...
VOID VerifyMemoryMapKey(IN UINTN MapKey); // See AMI porting comments in CoreExitBootServices
//*** AMI PORTING END *****//
...
// 3. ACPI FPDT table support (fill the exit boot services timings).
    ...
    // VerifyMemoryMapKey verifies MapKey is records verification results.
    // If MapKey is invalid, CoreTerminateMemoryMap call below will fail.
    VerifyMemoryMapKey(MapKey);
