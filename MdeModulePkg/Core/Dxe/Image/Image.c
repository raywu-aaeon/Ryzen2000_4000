   ...
   for (Index = 0; Index < NumberOfSections; Index++) {
     ...
     if ((SectionHeader.Characteristics & EFI_IMAGE_SCN_CNT_CODE) == 0) {
       ...
       if (ValueInSectionHeader != 0) {
         ...
         //*** AMI PORTING BEGIN ***//
         // Bug fix. since image-size is a full image size and EFI_SIZE_TO_PAGES rounds up,
         // there is no need to add <section-alignment>
         //
         ...
         Status = CheckAndMarkFixLoadingMemoryUsageBitMap (ImageContext->ImageAddress, (UINTN)(ImageContext->ImageSize ));
         //*** AMI PORTING END *****//
       }
       break;
     }
  ...
  )
{
  ...
  if (DstBuffer == 0) {
    ...
    if (Image->NumberOfPages != 0 &&
      ...
      Image->NumberOfPages = EFI_SIZE_TO_PAGES ((UINTN)Image->ImageContext.ImageSize );
