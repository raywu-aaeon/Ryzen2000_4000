...
#if defined(CRB_SKIP_SWSMIxFF_IN_ROOT_SMI_HANDLER) && (CRB_SKIP_SWSMIxFF_IN_ROOT_SMI_HANDLER == 1)
//*** AMI PORTING BEGIN ***//
...
#define FCH_SMI_REG10 0x10
#define FCH_SMI_REG80 0x80
#define FCH_SMI_REG84 0x84
#define FCH_SMI_REG88 0x88
#define FCH_SMI_REG90 0x90
...
// SMI status register and BIT map.
SMI_STATUS_REG CONST mSmiStatusSource[] = {
    {FCH_SMI_REG10,      Gpe},
    {FCH_SMI_REG80,      AllGevents},
    {FCH_SMI_REG84,      FchFakeSts0 | (Smbus0 | BIT13 | BIT14) | UsbSmi | PwrBtn},
    {FCH_SMI_REG88,      Slp_Type | BIT12 | (FchGppSerr0 | FchGppSerr1 | FchGppSerr2 | FchGppSerr3)},
    {FCH_SMI_REG90,      (ShortTimer | LongTimer) | IoTrapping0},
};

/**
  Determine this SMI need to be handled through Root SMI handler.
  Due to lot of SW SMI 0xFFFF triggered, but most of them were not need to be handled by Root SMI handle.
  Check SMI register status, and determine we can skip it or not.

  @retval TRUE      We could skip Root SMI handler.
  @retval FALSE     Other SMI was triggered, we need to handle it.
  ...
  )
{
    ...
    // If there is any SMI status bit(beside SWSMI) is set.
    // We need to handle it through Root SMI handle, and can't be skipped
    for (i = 0; i < sizeof(mSmiStatusSource)/sizeof(SMI_STATUS_REG); i++) {
        SmiStatus = MmioRead32 (ACPI_MMIO_BASE + SMI_BASE + mSmiStatusSource[i].SmiStatusReg);
        if((SmiStatus & mSmiStatusSource[i].SmiStatusBit) != 0) {
            return FALSE;
        }
    }

    // If the SMI status is related to SWSMI only and SWSMI data is 0xFFFF, we can skip Root SMI handler
    // If it is other's SWSMI, we need to handle it.
    SmiStatus = MmioRead32 (ACPI_MMIO_BASE + SMI_BASE + FCH_SMI_REG88);
    if((SmiStatus & SmiCmdPort) != 0){
        SwSmiValue = IoRead16(SW_SMI_IO_ADDRESS);
        if(SwSmiValue == 0xffff) {
            // Clear SwSmi status
            MmioWrite32 (ACPI_MMIO_BASE + SMI_BASE + FCH_SMI_REG88, BIT11);
            
            // Set Eos
            SmiStatus = MmioRead32 (ACPI_MMIO_BASE + SMI_BASE + FCH_SMI_REG98);
            SmiStatus |= Eos;
            MmioWrite32 (ACPI_MMIO_BASE + SMI_BASE + FCH_SMI_REG98, SmiStatus);
