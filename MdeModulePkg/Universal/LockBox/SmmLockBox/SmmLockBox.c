  ...
  )
{
  ...
  //
  //*** AMI PORTING BEGIN ***//
  //Bug fix: In certain cases both TempLockBoxParameterRestore.Buffer and TempLockBoxParameterRestore.Length are zeros.
  // This case is handled by the if statement below.
  // Since [0..0] is hardly a valid buffer, buffer validation should be performed only on non-zero buffers.
  // The following if statement is moved into the else-clause below that handles non-zero buffers.
