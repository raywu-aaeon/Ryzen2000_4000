...
typedef struct {
    UINT8   AcpiSpecVersion;
    UINTN   TableAddress;
} ACPI_REVISION_TABLE_POINTER_STRUCTURE;

ACPI_REVISION_TABLE Version64 [] = {
        {EFI_ACPI_6_3_DIFFERENTIATED_SYSTEM_DESCRIPTION_TABLE_SIGNATURE , 2},
        {EFI_ACPI_6_3_SECONDARY_SYSTEM_DESCRIPTION_TABLE_SIGNATURE , 2},
        {EFI_ACPI_6_3_MULTIPLE_APIC_DESCRIPTION_TABLE_SIGNATURE , 5},
        {SIGNATURE_32 ('G', 'I', 'C', 'C') , 0},
        {EFI_ACPI_6_3_SMART_BATTERY_SPECIFICATION_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_EMBEDDED_CONTROLLER_BOOT_RESOURCES_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_SYSTEM_LOCALITY_INFORMATION_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_SYSTEM_RESOURCE_AFFINITY_TABLE_SIGNATURE , 3},
        {EFI_ACPI_6_3_CORRECTED_PLATFORM_ERROR_POLLING_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_MAXIMUM_SYSTEM_CHARACTERISTICS_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_ACPI_RAS_FEATURE_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_MEMORY_POWER_STATE_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_PLATFORM_MEMORY_TOPOLOGY_TABLE_SIGNATURE , 2},
        {EFI_ACPI_6_3_BOOT_GRAPHICS_RESOURCE_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_FIRMWARE_PERFORMANCE_DATA_TABLE_SIGNATURE , 1},
        {EFI_ACPI_6_3_GENERIC_TIMER_DESCRIPTION_TABLE_SIGNATURE , 3},
        {EFI_ACPI_6_3_NVDIMM_FIRMWARE_INTERFACE_TABLE_STRUCTURE_SIGNATURE , 1},
        {EFI_ACPI_6_3_HETEROGENEOUS_MEMORY_ATTRIBUTE_TABLE_SIGNATURE , 2},
        {EFI_ACPI_6_3_PLATFORM_DEBUG_TRIGGER_TABLE_STRUCTURE_SIGNATURE , 0},
        {EFI_ACPI_6_3_PROCESSOR_PROPERTIES_TOPOLOGY_TABLE_STRUCTURE_SIGNATURE , 3},
        {SIGNATURE_32 ('P', 'H', 'A', 'T'), 1},
        {EFI_ACPI_6_3_SECURE_DEVICES_TABLE_SIGNATURE, 1},
        {0,0}
};

ACPI_REVISION_TABLE_POINTER_STRUCTURE AcpiRevision [] = {
        {64, 0},
        {0, 0}
};
#endif
  ...
  )
{
  ...
  UINT64                    Buffer64;
//----AMI PORTING BEGIN ----  
// Rise TPL to avoid executing of callbacks between installation of two ACPI tables v.1 and v.2
  EFI_TPL                   OldTpl;
//----AMI PORTING END ----
...
//----AMI PORTING END ----
  if ((Version & EFI_ACPI_TABLE_VERSION_1_0B) != 0) {
    ...
    if (EFI_ERROR (Status)) {
    //----AMI PORTING BEGIN ----  
    // Restore TPL, which was rose to avoid executing of callbacks between installation of two ACPI tables v.1 and v.2
  ...
  )
{
  ...
  ASSERT (Handle);
//----AMI PORTING BEGIN ----
//---If ACPI version >= 6.4 perform checks on versions of submitted tables
#if ACPI_SPEC_VERSION >= 64
  {
      ACPI_REVISION_TABLE* Entry;
      UINTN i, j;
      
      for (i=0; AcpiRevision[i].TableAddress != 0; i++)
      {
          if (AcpiRevision[i].AcpiSpecVersion > ACPI_SPEC_VERSION)
              continue;
          else
          {
              Entry = (ACPI_REVISION_TABLE*)(VOID*)(AcpiRevision[0].TableAddress);
              for (j=0; Entry[j].Signature != 0; j++ )
              {
                  if (Entry[j].Signature == ((EFI_ACPI_COMMON_HEADER *)Table)->Signature)
                  {
                      if (Entry[j].Revision != ((EFI_ACPI_DESCRIPTION_HEADER *)Table)->Revision)
                      {
                          DEBUG ((EFI_D_ERROR, "!!!!Wrong revision for table with signature: %x, \n !!!Revision submitted: %x should be: %x due to Spec %d \n", Entry[j].Signature, ((EFI_ACPI_DESCRIPTION_HEADER *) Table)->Revision, Entry[j].Revision, ACPI_SPEC_VERSION));
                      }
                  }
              }
          }
  ...
  )
{
  ...
  //----AMI PORTING BEGIN ----
#if ACPI_SPEC_VERSION >= 64
  AcpiRevision[0].TableAddress = (UINTN)(VOID*)&Version64;
#endif
  //----AMI PORTING END ----
  //
  // Check for invalid input parameters
