...
// MaxSize here is in CHAR16 units, not bytes
EFI_STATUS BuildCapsuleVariableName(CHAR16* CapsuleVarName, UINTN MaxSize)
{
    CHAR16 *TempVarName;  
    UINTN Size;
    UINTN Index;
    ...
    //
    Status = StrCpyS(CapsuleVarName, MaxSize, EFI_CAPSULE_VARIABLE_NAME);
    if (EFI_ERROR(Status))
        return Status;

    VarNameSize = StrLen(CapsuleVarName);      //in CHAR16 units
    TempVarName = CapsuleVarName + VarNameSize; //in CHAR16 units
    Remainder = (MaxSize - VarNameSize) * sizeof (CHAR16); //in bytes
    Index = 1;
    do{
        Size = 0;
        ...
        if (Status==EFI_BUFFER_TOO_SMALL){
            Status = UnicodeValueToStringS (TempVarName, Remainder, 0, Index, 0);
            if (EFI_ERROR(Status))
                return Status;
            Index++;
            continue;
  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // Enhancement: Support capsule scheduling from code that is not part of this driver.
  Status = BuildCapsuleVariableName(CapsuleVarName, sizeof(CapsuleVarName)/sizeof(CHAR16));
  if (EFI_ERROR(Status))
      return Status;
