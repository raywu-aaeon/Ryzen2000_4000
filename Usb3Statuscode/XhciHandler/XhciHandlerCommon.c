...
#else
#if defined __GNUC__
	__asm__ __volatile__ ("outb %b0,%w1" : : "a" (ChkPoint), "d" (0x80));
#else
	__asm {
			mov al,ChkPoint
			mov dx, 0x80
...
//<AMI_PHDR_START>
//--------------------------------------------------------------------
// Procedure:   GetDbgPeiXhciMmioSize
//
// Description: Returns the value of DBG_PEI_XHCI_MMIOSIZE macro.
//
// Input:       None
//
// Output:      UINTN
...
//--------------------------------------------------------------------
//<AMI_PHDR_END>
UINTN GetDbgPeiXhciMmioSize()
{
    return DBG_PEI_XHCI_MMIOSIZE;
}
