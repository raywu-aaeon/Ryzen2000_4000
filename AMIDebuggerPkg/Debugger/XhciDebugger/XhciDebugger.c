...
VOID LoadFirmwareVolume( IN CONST EFI_PEI_SERVICES ** , IN EFI_PHYSICAL_ADDRESS );
//<AMI_PHDR_START>
//--------------------------------------------------------------------
...
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
extern UINTN gRedirectionOnlyEnabled;
extern UINTN gPeiDebuggerEnabled;
EFI_STATUS 
DbgXhciInitialize (
		...
		IN CONST EFI_PEI_SERVICES  **PeiServices )
{
    ...
    if(!gRedirectionOnlyEnabled && gPeiDebuggerEnabled) {
          if(!IsPMModeEnabled(0) && SerData->m_IsPeiDbgIsNotS3)   //If boot is not S3 then perform LoadFirmwareVolume() 
              LoadFirmwareVolume(PeiServices, PeiDebugDataBaseAddress);
      }
    return EFI_SUCCESS;
}
