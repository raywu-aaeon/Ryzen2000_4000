...
// Validate Digest and returns Root CA cert or Signer Cert
        ...
        case Pkcs7CertValidateGetMatchInCertChain: // obsolete
            // non-0 reason argument forces Pkcs7_Chain_Validate to scan for matching Cert
            // among all the certs in a certificate chain regardless their link to a signer
            reason = -1; 
...
)
{
    ...
    if (pFwKeyHob!=NULL) {
        ...
        if(!EFI_ERROR(Status)) {
            Key->Blob = (UINT8*)((UINTN)pFwKeyHob+sizeof(FW_KEY_HOB));
// make sure the Key buffer is mapped to FV_BB address space 
/*
            if(!((UINTN)Key->Blob > FV_BB_BASE && 
                 (UINT64)((UINTN)Key->Blob+Key->BlobSize) < 
