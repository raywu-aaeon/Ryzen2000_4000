...
/**
   ...
   @param ghLen       The length of Hash data
       ...
       ++counter;
        ...
        switch(ghLen) {
            ...
            case SHA384_DIGEST_SIZE:
                sha384_vector(2, addr, (const size_t*)&len, buf);
                break;
            case SHA512_DIGEST_SIZE:
                sha512_vector(2, addr, (const size_t*)&len, buf);
                break;
#endif
            default: 
...
//   DEFAULT_RSA_KEY_MODULUS_LEN
   ...
   /* M = (eight) 0x00 || msghash || salt, mask = H(M) */
...
#ifdef LTC_SHA512
        case SHA384_DIGEST_SIZE:
            sha384_vector(3, addr, (const size_t*)len, mask);
            break;
        case SHA512_DIGEST_SIZE:
            sha512_vector(3, addr, (const size_t*)len, mask);
            break;
#endif
        default: 
            err = CRYPT_INVALID_HASH;
    ...
    wpa_printf(MSG_DEBUG, "PSS2: HashLen %X, err %d, Hash %X = %X",ghLen, err, *(size_t*)mask, *(size_t*)hash );
    if(err)
        goto LBL_ERR;
...
//    }
    else // SHA1/SHA385/512
    {
        err = (int)memcmp( (u8*)((size_t)Sig +((size_t)siglen-hashlen)), (u8*)hash, hashlen );
    }
