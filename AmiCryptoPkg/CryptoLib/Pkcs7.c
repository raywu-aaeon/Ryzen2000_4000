...
//            }
            ...
            // hash mismatch keep looking for cert in a chain
                ...
                if(os_memcmp(revokeInfo->ToBeSignedHash, hash, hashLen))
                    continue;    
            ...
            for (trust = trusted; trust; trust = trust->next) {
                ...
                // Msft Win10 SignedData may use intermediate certs as a Trust cert.
...
//                    !os_memcmp(signer->tbs_cert_start, trust->tbs_cert_start, trust->tbs_cert_len))
                    !os_memcmp(leaf->tbs_cert_start, trust->tbs_cert_start, trust->tbs_cert_len))
                {
