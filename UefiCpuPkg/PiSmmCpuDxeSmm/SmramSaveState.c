...
#include "Token.h"
#if defined(SMM_ISOLATION_SUPPORT) && (SMM_ISOLATION_SUPPORT == 1)
...
//(AMI AMD Porting) >> SMM isolation
extern BOOLEAN mPcdSmmIsolationSupport;
#if defined(SMM_ISOLATION_SUPPORT) && (SMM_ISOLATION_SUPPORT == 1)
#else
BOOLEAN
NeedSysCall (
...
//(AMI AMD Porting)>> For compile, since the original SMRAM_SAVE_STATE_MAP was replaced by AMD's SMRAM_SAVE_STATE_MAP in SmramSaveStateMap.h
...
//(AMI AMD Porting)<<
  ...
  //
//(AMI AMD Porting)>> Padding the index 2~6 for AMD 
...
//  {0, 8, 0                            , SMM_CPU_OFFSET (x64.LdtBaseLoDword) , SMM_CPU_OFFSET (x64.LdtBaseHiDword), FALSE},  //  EFI_SMM_SAVE_STATE_REGISTER_LDTBASE  = 6
  {0, 0, 0                            , 0                                   , 0                                  , FALSE},  // Reserved 2
  {0, 0, 0                            , 0                                   , 0                                  , FALSE},  // Reserved 3
  {0, 0, 0                            , 0                                   , 0                                  , FALSE},  // Reserved 4
  {0, 0, 0                            , 0                                   , 0                                  , FALSE},  // Reserved 5
  {0, 0, 0                            , 0                                   , 0                                  , FALSE},  // Reserved 6
//(AMI AMD Porting)<< 
...
{
//(AMI AMD porting)>> For intel only
//  UINT32                      SmmRevId;
...
//(AMI AMD porting)<< For intel only
  ...
  //  VOID                        *IoMemAddr;
  // AMI_OVERRIDE_END <<<
//(AMI AMD porting)>> Added AMD's IO trap function
  UINT32                       IoTrapOffset;
  SMRAM_SAVE_STATE_MAP	       *CpuSaveState;
...
/* (AMI AMD porting)>> This is for Intel only 
      if (!mPcdSmmIsolationSupport) {
      //
      // Get SMM Revision ID
      //
...
//(AMI AMD porting)>> Added AMD's IO trap function
        ...
        IoTrapOffset = CpuSaveState->x64.IoTrapOffset;

        if (!(IoTrapOffset & BIT1)) return EFI_NOT_FOUND;    //if not valid return

        IoInfo->IoData = CpuSaveState->x64._RAX;
        if(IoTrapOffset & BIT4){
            IoInfo->IoWidth = EFI_SMM_SAVE_STATE_IO_WIDTH_UINT8;
            //IoInfo->IoData = (*(UINT64*)(SmmBase + 0xfff8)) & 0xff;
            IoInfo->IoData &= 0xFF;
        }else if (IoTrapOffset & BIT5){
            IoInfo->IoWidth = EFI_SMM_SAVE_STATE_IO_WIDTH_UINT16;
            //IoInfo->IoData = (*(UINT64*)(SmmBase + 0xfff8)) & 0xffff;
            IoInfo->IoData &= 0xFFFF;
        }else{
            IoInfo->IoWidth = EFI_SMM_SAVE_STATE_IO_WIDTH_UINT32;
            //IoInfo->IoData = (*(UINT64*)(SmmBase + 0xfff8)) & 0xffffffff;
            IoInfo->IoData &= 0xFFFFFFFF;
        }

        IoInfo->IoPort = (UINT16)(IoTrapOffset >> 16);

        if(IoTrapOffset & BIT0)
            IoInfo->IoType = EFI_SMM_SAVE_STATE_IO_TYPE_INPUT;//In
        else
            IoInfo->IoType = EFI_SMM_SAVE_STATE_IO_TYPE_OUTPUT;//Out

        if(IoTrapOffset & BIT2){
            IoInfo->IoType = EFI_SMM_SAVE_STATE_IO_TYPE_STRING;//String
            //IO Restart RSI has the source value.
            //IoInfo->IoData = *(UINT64*)(SmmBase + 0xfeb0); //Get RSI Value
            IoInfo->IoData = CpuSaveState->x64.IoRestartRsi;
        }
        if(IoTrapOffset & BIT3)
            IoInfo->IoType = EFI_SMM_SAVE_STATE_IO_TYPE_REP_PREFIX;//Repeat

    } else {

        if (NeedSysCall () == TRUE) {
            IoTrapOffset = (UINT32) SysCall (SMM_SC_SVST_READ, CpuIndex, (UINTN) ((UINT8 *)CpuSaveState + SMM_CPU_OFFSET (x64.IoTrapOffset)), 4);
        } else {
            IoTrapOffset = CpuSaveState->x64.IoTrapOffset;
        }
        ...
        if (NeedSysCall () == TRUE) {
            IoInfo->IoData = (UINT64) SysCall (SMM_SC_SVST_READ, CpuIndex, (UINTN) ((UINT8 *)CpuSaveState + SMM_CPU_OFFSET (x64._RAX)), 8);
        } else {
            IoInfo->IoData = CpuSaveState->x64._RAX;
        }
        ...
        if(IoTrapOffset & BIT2){
            ...
            //IoInfo->IoData = *(UINT64*)(SmmBase + 0xfeb0); //Get RSI Value
            if (NeedSysCall () == TRUE) {
                IoInfo->IoData = (UINT32) SysCall (SMM_SC_SVST_READ, CpuIndex, (UINTN) ((UINT8 *)CpuSaveState + SMM_CPU_OFFSET (x64.IoRestartRsi)), 8);
            } else {
                IoInfo->IoData = CpuSaveState->x64.IoRestartRsi;
            }
        }
...
//(AMI AMD Porting) >> SMM isolation
  ...
  if ((SmmSupervisor != NULL) && (SmmEntryAddr != NULL)) {
