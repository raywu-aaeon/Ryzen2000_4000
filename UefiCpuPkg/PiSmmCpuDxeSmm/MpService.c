...
//(AMI AMD Porting) >> This is for S3.
//1. System entering S3 in FCH at SMM phase
//2. System resumes from S3 at SMM phase and leaves SMM with RSM directly in BspSmmResumeVector/ApSmmResumeVector
//3. The SMM flags are not cleared to the default value on Step 2.
//4. BSP and AP have unexpected behavior when SMI occurs on S3 because SMM flags are in the state of Step 1
//5. Use a Smm protocol which is installed here for initializing the SMM flags in BspSmmResumeVector/ApSmmResumeVector
extern  EFI_GUID    gAmiMpSyncDataProtocolGuid;
//(AMI AMD Porting) <<
//(AMI AMD porting) >> SMM isoation
  ...
  )
{
  ...
  //(AMI AMD Porting) >> This is for S3 resume. See the description on the top of this file.
  EFI_HANDLE                SmmMpSyncHandle = NULL;
  //(AMI AMD Porting) <<
...
//(AMI AMD Porting)<<
//(AMI AMD porting) >> This is for non SMM S3
  gRT->SetVariable (L"AmiMpSyncData", &gAmiMpSyncDataGuid,
          EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS,
          sizeof (UINT32), &mSmmMpSyncData);
//(AMI AMD Porting)<<
  return Cr3;
}
