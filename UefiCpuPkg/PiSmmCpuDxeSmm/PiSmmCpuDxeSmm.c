...
// AMI_OVERRIDE_END <<<
//(AMI AMD Porting) >> SMM isolation
BOOLEAN mPcdSmmIsolationSupport = FALSE;
//(AMI AMD Porting) <<
/**
...
//(AMI AMD Porting) mSmmS3Flag is not used      if (!mSmmS3Flag) {
        //
...
{
//(AMI AMD Porting) AMD doesn't have Acpi Cpu data GetAcpiCpuData ();
  ...
  )
{
  ...
  UINT8                      *Stacks;
  VOID                       *Registration;
  //(AMI AMD porting) The family id and model id are for Intel CPU >>
/*UINT32                     RegEax;
  UINT32                     RegEdx;
  UINTN                      FamilyId;
  UINTN                      ModelId;*/
  ...
  UINT32                     Cr3;
//(AMI AMD Porting) >> SMM isolation
  if (FeaturePcdGet(PcdSmmIsolationSupport)) {
...
//(AMI AMD porting) AMD CPU don't need this flag  GetAcpiS3EnableFlag ();
//(AMI AMD porting) This will be initialed at PiSmmCommunicationSmm.c InitSmmS3ResumeState (Cr3);
  ...
  )
{
  if (mSmmReadyToLock) {
    ...
    if (!FeaturePcdGet(AmiPcdSmmPageProtectionEnable))
    {
...
{
//(AMI AMD Porting) AMD doesn't need this RestoreSmmConfigurationInS3 ();
}
