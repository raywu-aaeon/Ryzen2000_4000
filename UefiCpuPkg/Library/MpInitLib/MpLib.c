...
//(AMI AMD Porting) >> Get MTRR Hob that is created by MemDetect.c for initializing MTRR
extern VOID InitializeMtrr();
//(AMI AMD Porting) <<
/**
  ...
  )
{
  ...
  //(AMI AMD Porting) >> AMD doesn't support Mwait function.
  return FALSE;
  //(AMI AMD Porting) <<
  ...
  )
{
  ...
  while (TRUE) {
    if (CpuMpData->InitFlag == ApInitConfig) {
      ...
      //
      // (AMI AMD Porting)>> AMD system doesn't need to do microcode update and MTRR sync here
  ...
  )
{
  ...
  //(AMI AMD Porting)>> Get the maximum CPU number
  UINT32                   RegEcx;
  //(AMI AMD Porting)<<
  ...
  //(AMI AMD Porting)>> 
      // find a way to shrink the size of BufferSize, since it will affect the S3 resume memory size
      // The S3_BASE_MEMORY_SIZE needs to be enlarged if using the default PcdCpuMaxLogicalProcessorNumber
      AsmCpuid(0x80000008, NULL, NULL, &RegEcx, NULL);
      MaxLogicalProcessorNumber = (RegEcx & 0x0FF) + 1;
      DEBUG ((DEBUG_INFO, "MaxLogicalProcessorNumber %x\n", MaxLogicalProcessorNumber));
  ...
  *(UINTN*)(ApIdtBase - sizeof(UINTN)) = *(UINTN*)(VolatileRegisters.Idtr.Base - sizeof (UINTN));
  ...
  //(AMI AMD Porting) >> Get MTRR Hob that is created by MemDetect.c so that MtrrGetAllMtrrs can get correct MTRR setting.
  InitializeMtrr();
  //(AMI AMD Porting) <<
  //
