...
//(AMI AMD Porting)>> Save AP reset vector address
EFI_PHYSICAL_ADDRESS    mStartAddress = 0;
//(AMI AMD Porting)<<
//(AMI AMD porting)>>
//  The APICSWEN bit should be cleared in AMD system.
//  This is for fixing below issues
//  1. CC6 issue.
//  2. OS installing when SMM isolation enabled.

UINT32  ReadLocalApicReg(IN UINTN  MmioOffset);
VOID    WriteLocalApicReg (
  IN UINTN  MmioOffset,
  IN UINT32 Value
  );
//(AMI AMD porting)<<
  ...
  )
{
  ...
  //(AMI AMD Porting)>> Use the AP reset vector address saved before
  if (mStartAddress != 0)
  {
      return (UINTN) mStartAddress;
  }
  //(AMI AMD Porting)<<
  ...
  if (!EFI_ERROR (Status)) {
/* (AMI AMD Porting)>> Do not free the memory, since this memory is used for waking AP up
    ...
    ASSERT_EFI_ERROR (Status);
(AMI AMD Porting)<<*/
//(AMI AMD Porting)>> Save the AP reset vector address in global variable
...
//  The APICSWEN bit should be cleared in AMD system.
//  This is for fixing below issues
//  1. CC6 issue when SW downcore in OS.
//  2. OS installing when SMM isolation enabled.
  LOCAL_APIC_SVR      Svr;

  Svr.Uint32 = ReadLocalApicReg (XAPIC_SPURIOUS_VECTOR_OFFSET);
  Svr.Bits.SpuriousVector = 0x0f;
