...
/** @file AmiIoApicLib.c
    This file contains DXE stage Io Apic component code for
    Template SB
...
{
  ASSERT (Index < 0x100);
  MmioWrite8 (APCB + IOAPIC_INDEX_OFFSET, (UINT8)Index);
  return MmioRead32 (APCB + IOAPIC_DATA_OFFSET);
}
  ...
  )
{
  ASSERT (Index < 0x100);
  MmioWrite8 (APCB + IOAPIC_INDEX_OFFSET, (UINT8)Index);
  return MmioWrite32 (APCB + IOAPIC_DATA_OFFSET, Value);
}
  ...
  )
{
  ...
  Entry.Bits.Polarity        = AssertionLevel ? 1 : 0;
