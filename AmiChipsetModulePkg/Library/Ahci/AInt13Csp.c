...
)
{
    ...
    if ((!EFI_ERROR(Status)) && ( (UINT16)(VendorDeviceID) == 0x1022)){ //AMD Platform
        UINTN       PciSeg, PciBus, PciDev, PciFunc;
        PciIo->GetLocation( PciIo, &PciSeg, &PciBus, &PciDev, &PciFunc );
        if (((UINT16)(VendorDeviceID >> 16) & 0xFF00)== 0x7900) {
            //SOC's SATA 
            //Update the AccessIndexPort/AccessDataPort
            
            *RegOffset = 0xD8;
            
            *Device  = PciDev;// IDP index port | Device |Function 
            *Funtion  = PciFunc;// IDP index port | Device |Function 
            *Bus = PciBus; //  IDP Data port replace it with bus number
