    ...
    IN OUT OFBD_HDR *pOFBDHdr)
{
	...
	if (OCStructPtr->dwFlaSts == OFBD_TC_OEM_CMD_BF) {	 
	    ...
	    OfbdStrCpy((char *)pOFBDTblEnd, DisOemBFStr);
	...
	// 02. After Flash
	    ...
	    pOFBDHdr->OFBD_RS |= OFBD_RS_DIS_OEMSTR;
	    // copy the oem string ptr for AFU
	    OfbdStrCpy((char *)pOFBDTblEnd, DisOemAFStr);
	...
	// 03. Before End
	    ...
	    pOFBDHdr->OFBD_RS |= OFBD_RS_DIS_OEMSTR;
	    // copy the oem string ptr for AFU
	    OfbdStrCpy((char *)pOFBDTblEnd, DisOemBEStr);
	    ...
	    pOFBDHdr->OFBD_RS |= OFBD_RS_DIS_OEMSTR;
	    // copy the oem string ptr for AFU
	    OfbdStrCpy((char *)pOFBDTblEnd, DisOemSFStr);
