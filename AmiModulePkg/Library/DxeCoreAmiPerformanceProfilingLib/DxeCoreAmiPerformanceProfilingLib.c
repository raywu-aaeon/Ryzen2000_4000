  ...
  )
{
    ...
    if(ProfilingName != NULL && Handle != NULL)
    {
        ...
        else if(Status == EFI_NOT_FOUND)
        {
            ...
            Status = AsciiStrnCpyS(EntryList[CurrentEntry].Interface, AMI_PERFORMANCE_PROFILING_STRING_SIZE, ProfilingName, AMI_PERFORMANCE_PROFILING_STRING_SIZE);
            if (EFI_ERROR(Status))
            {
                return Status;
            }
        ...
        if(EntryList!=NULL)
        {
            ...
            if(Hob != NULL)
            {
                ...
                for(i = 0; i < AMI_PERFORMANCE_PROFILING_INITIAL_ENTRIES_PEI ; i++)
                {
                    ...
                    {
                        Status = AsciiStrnCpyS(EntryList[CurrentEntry].Interface, AMI_PERFORMANCE_PROFILING_STRING_SIZE, Entry[i].Interface, AMI_PERFORMANCE_PROFILING_STRING_SIZE);
                        // There is an issue with the Interface string move on to the next entry
                        if (EFI_ERROR(Status))
                        {
                            continue;
                        }
