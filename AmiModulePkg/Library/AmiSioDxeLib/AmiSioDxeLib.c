...
//------------------	
	if(Volatile){
	    ...
	    UnicodeSPrint(s, StrSize, LD_VV_VAR_NAME_FORMAT, SioLd->DeviceInfo->PnpId, SioLd->DeviceInfo->Uid);
#else
	    UnicodeSPrint(s, StrSize, LD_VV_VAR_NAME_FORMAT, SioLd->Owner->SioInfoIdx, SioLd->DevInfoIdx);
#endif		
	} else {
#if (SIO_SETUP_USE_APTIO_4_STYLE==1)
	    UnicodeSPrint(s, StrSize, LD_NV_VAR_NAME_FORMAT, SioLd->DeviceInfo->PnpId, SioLd->DeviceInfo->Uid);
#else
	    UnicodeSPrint(s, StrSize, LD_NV_VAR_NAME_FORMAT, SioLd->Owner->SioInfoIdx, SioLd->DevInfoIdx);
#endif
	}
	
	ret=MallocZ(Wcslen(s)*sizeof(CHAR16)+sizeof(CHAR16));
	StrCpyS(ret, Wcslen(s) + 1, s);
	return ret;
}
