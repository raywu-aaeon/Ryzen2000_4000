...
{
    VA_LIST   ArgList;
    AMI_DEBUG_SERVICE_PROTOCOL *DebugServiceProtocol = GetAmiDebugServicesProtocolInstance();

    if(DebugServiceProtocol == NULL || (ErrorLevel & GetDebugPrintErrorLevel ()) == 0) return;

    VA_START(ArgList,Format);
    DebugServiceProtocol->Debug(ErrorLevel, Format, ArgList);
    VA_END(ArgList);
}
  ...
  )
{
    ...
    if(DebugServiceProtocol == NULL) return;
    DebugServiceProtocol->DebugAssert(FileName, LineNumber, Description);
