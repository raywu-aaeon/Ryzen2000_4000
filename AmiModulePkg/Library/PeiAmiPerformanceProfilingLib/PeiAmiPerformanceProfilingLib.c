        ...
        else if(Status == EFI_NOT_FOUND)
        {
            ...
            if(Entry != NULL)
            {
                for(i = 0; i < AMI_PERFORMANCE_PROFILING_INITIAL_ENTRIES_PEI; i++)
                {
                    ...
                    {
                        Status = AsciiStrnCpyS(Entry[i].Interface, AMI_PERFORMANCE_PROFILING_STRING_SIZE, ProfilingName, AMI_PERFORMANCE_PROFILING_STRING_SIZE);
                        if (EFI_ERROR(Status))
                        {
                            // error copying the Interface data over.  skip this entry and move on to the next
                            continue;
                        }
