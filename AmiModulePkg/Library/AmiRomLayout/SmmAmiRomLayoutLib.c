    ...
    if(AmiRomLayoutHob != NULL)
    {
        ...
        if(gAmiRomLayout!=NULL)
        {
            ...
            if(InstallSmmConfigurationTable)
            {
                ...
                ConfigurationTable = AllocateCopyPool(AmiRomLayoutHob->Header.Header.HobLength, AmiRomLayoutHob);
                if (ConfigurationTable!=NULL)
                    gSmst->SmmInstallConfigurationTable(gSmst, &gAmiRomLayoutGuid, ConfigurationTable, AmiRomLayoutHob->Header.Header.HobLength);
            }
        }
    }
    return EFI_SUCCESS;
