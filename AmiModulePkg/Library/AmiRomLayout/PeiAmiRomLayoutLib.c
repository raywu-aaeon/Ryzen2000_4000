...
EFI_STATUS AmiGetRomLayout(
    ...
    Status = PeiServicesAllocatePool(TotalSize, (VOID**)RomLayout);
    if (EFI_ERROR(Status)) return Status;
...
EFI_STATUS AmiPublishFvArea(IN AMI_ROM_AREA *FvArea)
{
    ...
    if(FvArea != NULL)
    {
        ...
        if(Fv->Signature == EFI_FVH_SIGNATURE)
        {
            ...
            PeiServicesInstallFvInfoPpi(&(((EFI_FIRMWARE_VOLUME_HEADER*)(UINTN)ReadUnaligned64(&FvArea->Address))->FileSystemGuid),
