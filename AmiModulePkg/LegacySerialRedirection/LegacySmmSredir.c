...
)
{
    UINT8       i;
    UINT32      TempValue=0;
    UINT8       *DataBuffer=(UINT8*)((UINTN)SredirParam->BufferAddress);
    EFI_STATUS  Status;
...
/**
    ...
    @param[in] Protocol   Points to the EFI_ACPI_EN_DISPATCH_PROTOCOL_GUID.
    @param[in] Interface  Points to the EFI_ACPI_DISPATCH_PROTOCOL's interface instance.
...
)
{
    ...
    Status = pSmst->SmmLocateProtocol(&gEfiAcpiDisDispatchProtocolGuid, NULL, (VOID**)&AcpiDisDispatch);
    if (EFI_ERROR(Status)) {
        return Status;
    }
