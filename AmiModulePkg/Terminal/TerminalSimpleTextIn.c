...
)
{
    ...
    if(TerminalDev->EscSeqStatePtr == NULL) {
        Status = pBS->SetTimer(TerminalDev->TimeoutEscEvent, TimerCancel, 0);
...
)
{
    ...
    if (TerminalDev->EscSeqStatePtr == NULL) {
        ...
        Status = pBS->SetTimer(TerminalDev->TimeoutEvent,
                        TimerRelative,
    ...
    if (EFI_ERROR(pBS->CheckEvent(TerminalDev->TimeoutEvent)))
    {
        ...
        Status = pBS->SetTimer(TerminalDev->TimeoutEvent, TimerCancel, 0);
        if(EFI_ERROR(Status)){
            return Status;
        }
        if (TerminalDev->EscSeqStatePtr == NULL) {
...
)
{
    ...
    if (IsTerminalKeyboardBufferEmpty (&(TerminalDev->KeyboardBuffer))) {
        ...
        if(!EFI_ERROR(Status) && TerminalDev->EscSeqStatePtr != NULL) {
            ...
            Status = pBS->SetTimer(TerminalDev->TimeoutEscEvent,
                        TimerPeriodic,
...
)
{
    ...
    BOOLEAN             ShiftKeyMatch;
    BOOLEAN             CtrlKeyMatch;
    BOOLEAN             AltKeyMatch;
    BOOLEAN             LogoKeyMatch;
    BOOLEAN             MenuKeyMatch;
    BOOLEAN             SysRqKeyMatch;
...
)
{
    ...
    *NotifyHandle = (EFI_HANDLE) (&mTerminalKeyboardRecord->Link);
    ...
    Status = pBS->SetTimer(TerminalDev->TerminalKeyEvent,
                        TimerPeriodic,
