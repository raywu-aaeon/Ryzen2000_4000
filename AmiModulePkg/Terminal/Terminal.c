...
extern EFI_GUID gTerminalPortsVarGuid;
extern TERMINAL_PORTS_ENABLED gTerminalPortsEnabled;
extern UINTN  gTerminalPortsEnabledVarSize;
    ...
    IN EFI_DEVICE_PATH_PROTOCOL     *RemainingDevicePath)
{
    ...
    if ((SerialIo->Revision >= EFI_SERIAL_IO_PROTOCOL_REVISION1p1) && (SerialIo->DeviceTypeGuid != NULL)) {
        if (!CompareGuid(SerialIo->DeviceTypeGuid, &gEfiSerialTerminalDeviceTypeGuid)) {
            pBS->CloseProtocol(
                        Controller,
                        &gEfiSerialIoProtocolGuid,
                        This->DriverBindingHandle,
...
)
{
    ...
    if (!ConsoleRedirectionEnable) {
        return EFI_DEVICE_ERROR;
    }

    if (Disable_Terminal_For_SCT_Test) {
        return EFI_DEVICE_ERROR;
    }
...
Error_Devicepath:
    ...
    if(!EFI_ERROR(Status)) {
    	// Updating the TERMINAL_PORTS_ENABLED_VAR only when valid ports are found
...
)
{
    ...
    if(ResourcesList != NULL){
        for(Index = 0; Index < ResourcesList->ItemCount; Index++){
            SmlHdr = (ACPI_SMALL_RESOURCE_HEADER*)ResourcesList->Items[Index];
            if (SmlHdr->Bits.Type == ACPI_SMALL_ITEM_FLAG) {
                switch(SmlHdr->Bits.Name) {
                case ACPI_SMALL_FIXED_IO_PORT_DESCRIPTOR_NAME:
                    SpcrInfo->BaseAddress=((EFI_ACPI_FIXED_LOCATION_IO_PORT_DESCRIPTOR*)SmlHdr)->BaseAddress;
                ...
                LarHdr = (ACPI_LARGE_RESOURCE_HEADER*)ResourcesList->Items[Index];
                switch(LarHdr->Header.Bits.Name) {
                case ACPI_LARGE_EXTENDED_IRQ_DESCRIPTOR_NAME:
                    IrqNo = (UINT8)(((EFI_ACPI_EXTENDED_INTERRUPT_DESCRIPTOR*)LarHdr)->InterruptNumber[0]);
                    SpcrInfo->Irq = IrqNo;
...
)
{
    ...
    if(spcr_global_system_int == MAX_UINT32) {
        spcr_tbl->GlobalSystemInt = SpcrInfo->Irq;
    } else {
        spcr_tbl->GlobalSystemInt = spcr_global_system_int;
    }
