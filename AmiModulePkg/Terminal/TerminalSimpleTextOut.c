...
RefreshScreen ()
{
    ...
    for(Index=0; Index<TotalTerminalDev;Index++) {
        ...
        for(i=0; i<MaxRows;i++) {
         ...
         TerminalSetCursorPosition(This, CurrentColumn, CurrentRow);
    }
}
...
)
{
    ...
    UnicodeSPrint(
               SetModeEscapeSequence+1, 
               (sizeof(SetModeEscapeSequence)-sizeof(CHAR16)), 
               L"[8;%d;%dt", 
               MaxRows,
               MaxColumns);

    Status = TerminalOutputEscapeSequence(This, SetModeEscapeSequence);
	if(EFI_ERROR(Status)){
	   return Status;
    }
        ...
        if (TerminalDev->TerminalType == VT100) {
            if (Blink) {
                UnicodeSPrint(
                        ...
                        sizeof(SetAttributeEscapeSequence), 
                        L"%c[%dm", 
                        ASCII_ESC, 
                        BlinkSequence);
...
)
{
    ...
    UnicodeSPrint (
            ...
            sizeof(SetCursorPositionEscapeSequence), 
            L"%c[%d;%dH", 
            ASCII_ESC, 
            (UINT8)Row+1,
            (UINT8)Column+1);
