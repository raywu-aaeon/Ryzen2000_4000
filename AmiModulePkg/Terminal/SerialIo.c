...
{
    {
        MESSAGING_DEVICE_PATH,
        MSG_VENDOR_DP,
        {sizeof (UART_FLOW_CONTROL_DEVICE_PATH),0}
    },
...
//------------------------------------
    ...
    if (!EFI_ERROR(Status)) {
        ...
        if(Pci) { 
            ...
            Parameters->ControllerName = (CHAR16 *) AllocateZeroPool(CONTROLLER_NAME_LENGTH * sizeof(CHAR16));
            UnicodeSPrint (
                    Parameters->ControllerName, 
                    CONTROLLER_NAME_LENGTH * sizeof(CHAR16), 
                    L"COM (Pci Dev%X, Func%X, Port%X)", 
                    Device, 
                    Function, 
                    Port);
            //
    ...
    if(!EFI_ERROR(Status)) {
        ...
        if (ResourceList.SmallHeader){
            for(SmlHdr=ResourceList.SmallHeader; (SmlHdr->Byte)!=ACPI_END_TAG_DESCRIPTOR; 
...
SerialTest:
    ...
    //
    Parameters->SerialIo.Revision       = EFI_SERIAL_IO_PROTOCOL_REVISION1p1;
    ...
    Parameters->SerialIo.DeviceTypeGuid = NULL;
    //
...
Error:
    ...
    DEBUG((DEBUG_INFO,"SerialIo: Close Protocol Status : %r\n",Status));
ErrorEnd:
...
)
{
    ...
    //
    if ((DataBits >=6) && (StopBits == OneFiveStopBits)) {
        return EFI_INVALID_PARAMETER;
    }
    ...
    if((SerialWriteErrorCheck && Parameters->SerialPortErrorCount >= MaximumSerialWriteErrorCount) || (Parameters->FlowCtrlCTSnotSet)) {
        *BufferSize = 0;
        ...
        if( Parameters->TimerEventActive == FALSE) {
            ...
            //
            Status = pBS->SetTimer(Parameters->SerialErrorReEnableEvent,
                            TimerPeriodic,
                            10000000);
            if(EFI_ERROR(Status)) {
                return Status;
