...
)
{
    ...
    if ( DeviceIndex > (Atapi_BlkIoDev->DeviceCount)) {
        return EFI_INVALID_PARAMETER;
    }

    if ( !Atapi_BlkIoDev->DeviceInfo[DeviceIndex-1]->LookedForMedia ) {
        Status = Atapi_DetectMedia(Atapi_BlkIoDev->DeviceInfo[DeviceIndex-1]);
        if ( Status != EFI_SUCCESS ) {
            return EFI_DEVICE_ERROR;
        }
        Atapi_BlkIoDev->DeviceInfo[DeviceIndex-1]->LookedForMedia = TRUE;
    }

    *MediaInfo = Atapi_BlkIoDev->DeviceInfo[DeviceIndex-1]->MediaInfo;
...
)
{
    ...
    Status = Atapi_ReadSectors(
                            Atapi_BlkIoDev->DeviceInfo[DeviceIndex-1],
                            Buffer,
                            StartLba,
                            NumberOfBlocks,
                            BlockSize
...
)
{
    ...
    // As per PI spec, DeviceIndex is a number from one to Number of BlockDevices
    if ( DeviceIndex > (Ata_BlkIoDev->DeviceCount)) {
        return EFI_INVALID_PARAMETER;
    }

    if ( !Ata_BlkIoDev->DeviceInfo[DeviceIndex-1]->LookedForMedia ) {
        Ata_BlkIoDev->DeviceInfo[DeviceIndex-1]->LookedForMedia = TRUE;

    }

    *MediaInfo = Ata_BlkIoDev->DeviceInfo[DeviceIndex-1]->MediaInfo;
...
)
{
    ...
    Status = Ata_ReadSectors(
                        Ata_BlkIoDev->DeviceInfo[DeviceIndex-1],
                        Buffer,
                        StartLba,
                        NumberOfBlocks,
                        BlockSize
