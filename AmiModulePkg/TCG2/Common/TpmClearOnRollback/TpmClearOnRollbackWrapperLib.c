...
EFI_STATUS RecoverySetPhysicalPresence()
{
    ...
    PpCmd.Hdr.Tag       = SwapBytes16( TPM_TAG_RQU_COMMAND );
    PpCmd.Hdr.ParamSize = SwapBytes32( sizeof (PHYSICAL_PRESENCE_CMD));
    PpCmd.Hdr.Ordinal   = SwapBytes32( TSC_ORD_PhysicalPresence );
    PpCmd.Data = SwapBytes16(TPM_PHYSICAL_PRESENCE_CMD_ENABLE );
    ...
    PpCmd.Data  = SwapBytes16(TPM_PHYSICAL_PRESENCE_PRESENT);
...
EFI_STATUS RecoveryEnableActivate()
{
    ...
    cmdEnable.Tag       = SwapBytes16( TPM_TAG_RQU_COMMAND );
    cmdEnable.ParamSize = SwapBytes32( sizeof (TPM_1_2_CMD_HEADER));
    cmdEnable.Ordinal   = SwapBytes32( TPM_ORD_PhysicalEnable );
    ...
    if(!EFI_ERROR(Status) && retHeader.RetCode == 0)
    {
        ...
        Command.Hdr.ParamSize = SwapBytes32( sizeof (DATA_CMD));
        Command.Hdr.Ordinal   = SwapBytes32( TPM_ORD_PhysicalSetDeactivated );
        Command.Data          = 0x0;
...
UINT8 RecoverySendClearCmd()
{
    ...
    cmdClear.Tag       = SwapBytes16( TPM_TAG_RQU_COMMAND );
    cmdClear.ParamSize = SwapBytes32( sizeof (TPM_1_2_CMD_HEADER));
    cmdClear.Ordinal   = SwapBytes32( TPM_ORD_ForceClear );
    ...
    else if((retHeader.RetCode == SwapBytes32(DISABLED))||
            (retHeader.RetCode == SwapBytes32(DEACTIVATED)))
    {
...
EFI_STATUS GetCapabilityCmd(TPM_Capabilities_PermanentFlag *PermFlags)
{
    ...
    if(PermFlags->RetCode == SwapBytes32(INVALID_POSTINIT))
    {
...
EFI_STATUS GetVolatileFlags(TPM_VOLATILE_FLAGS *VolatileFlags)
{
    ...
    cmdGetCap.subCapSize  = SwapBytes32( 4 ); // subCap is always 32bit long
    cmdGetCap.subCap      = SwapBytes32( TPM_CAP_FLAG_VOLATILE  );

    SendTpmCommandRecovery((UINT8 *)&cmdGetCap, sizeof(TPM_GetCapabilities_Input),
                           (UINT8 *)VolatileFlags, sizeof(TPM_VOLATILE_FLAGS));

    if(VolatileFlags->RetCode == SwapBytes32(INVALID_POSTINIT))
    {
        return EFI_NOT_READY;
    }
