                  ...
                  IN TCG_CONFIGURATION    *ConfigFlags)
{
  ...
  CONST EFI_PEI_SERVICES    **PolicyPeiServices = NULL;
  //
  //
  
  if(PeiServices == NULL){
      PolicyPeiServices = GetPeiServicesTablePointer ();
  }else{
      PolicyPeiServices = (CONST EFI_PEI_SERVICES **)PeiServices;
  }
  
  Status = (*PolicyPeiServices)->GetHobList((CONST EFI_PEI_SERVICES    **)PolicyPeiServices, (VOID**)&pHobList);
...
)
{
  ...
  if (TcgPolicyHob != NULL && !EFI_ERROR(Status))
  {
      ...
      TcgPolicyHob->ConfigInfo.PcrBanks               = (SetupData.Sha1 & PcdGetBool(PcdAmiSha1PCRPolicy)) | SetupData.Sha256 | SetupData.Sha384 |\
