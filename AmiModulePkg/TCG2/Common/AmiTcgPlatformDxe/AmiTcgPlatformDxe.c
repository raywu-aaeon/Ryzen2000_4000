    ...
    IN VOID      *ctx )
{
    ...
    if(EFI_ERROR(Status)){
        DEBUG((DEBUG_INFO, "MeasurePciOptionRom Failed\n"));
    }
    ...
    IN UINTN            MaxDestStrLen)
{
    ...
    for ( i = 0, r = erqst, j = 0; i <= PPI_MAX_BASIC_OP; i++, r >>= 2 )
    {
        if ( r & PPI_FEATURE_CHANGE )
        {
            ...
            Status = StrCatS( DesStr + StrLen( DesStr ), MaxDestStrLen,  String );  
            j++;
            gBS->FreePool(String);
            
            if(EFI_ERROR(Status)){  //check status from StrCat
               return;
            } 
            ...
            if ( j < count )
            {
                if(EFI_ERROR(StrCatS( DesStr + StrLen( DesStr ), MaxDestStrLen,  strDelim )))
                    return;
            }
    ...
    IN UINTN        MaxDesStrLen)
{
    ...
    if(erqst == TCPA_PPIOP_SETNOPPIPROVISION_TRUE)
    {
        ...
        Status = StrCatS( DesStr + StrLen( DesStr ), MaxDesStrLen, String );
    ...
    IN int rqst )
{
    ...
    gBS->SetMem( StrBuffer, sizeof (CHAR16) * MaxStrBuffer, 0 );
...
//CurX, CurY;
    ...
    Status = StrCatS( StrBuffer, MaxStrBuffer, String );
    ...
    if (rq < TCPA_PPIOP_SETNOPPIPROVISION_FALSE || rq > TCPA_PPIOP_SETNOPPIMAINTENANCE_TRUE)
    {
        Status =  GetStringFromToken( STRING_TOKEN( STR_TCG_CONFIGURATION ), &String );
        if(EFI_ERROR(Status)) return;
        Status = StrCatS( StrBuffer + StrLen( StrBuffer ), MaxStrBuffer, String );
        ...
        displOperations( erqst, count, StrBuffer, MaxStrBuffer );
    ...
    {
        displTpmNvOperations( rq , StrBuffer, MaxStrBuffer );
    }
