...
)
{
    ...
    pBS->SetMem(gStorageSecurityProtocol, sizeof(AMI_STORAGE_SECURITY_COMMAND_PROTOCOL), 0);
...
)
{
    ...
    if ( Status == EFI_SUCCESS )
        {
            if (StorageSecurityInterface != NULL ) {
                    if (StorageSecurityInterface->SupportedProtocolList != NULL) {
                        pBS->FreePool( StorageSecurityInterface->SupportedProtocolList );
                        StorageSecurityInterface->SupportedProtocolList = NULL;
                    }
