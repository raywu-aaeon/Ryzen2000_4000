...
EFI_STATUS EFIAPI AtadSmiHandler ()
{
...
EFI_STATUS EFIAPI AtadSmiHandler2 (
		IN EFI_HANDLE	DispatchHandle,
...
)
{
    ...
    Status = AmiValidateMemoryBuffer(gAtadBuffer, sizeof(ATAD_VARIABLE));
    if (EFI_ERROR(Status)){ 
        DEBUG((EFI_D_ERROR, "AtadSmiSmmSwDispatchCallback Memory Validation Failed! \n"));
        gAtadBuffer = NULL;
        return Status;
    }
...
)
{
    ...
    Status = AtadSmiSmmSwDispatchCallback ();
