...
// $Date: 1/23/14 5:25p $
        ...
        Device(TAAD)
        {
            ...
            Method(_DSW, 3) {
                ...
                if( LEqual(Arg0, 0) ) {
                    And(PMEN, 0xFBFF, PMEN) // Resetting bit 10
                }
                if( LEqual(Arg0, 1) ) {// Enable the device's wake capabilities
                    if( LLessEqual(Arg1, 4) ){
                        Or(PMEN, 0x400, PMEN)// Setting bit 10
                    }                    
                }
                return
            }
            ...
            {   
                ...
                Store (2047, TMZN)      //ATAD Time Zone Issue Workaround
                Store (TMZN, TZN)
                Store (DAYL, DYL)
            ...
            Method (_STV, 0x2, Serialized)
            {
                ...
                //  and check if its greater than 23
                if( LGreater(Add(FromBCD(HOR), THOR, THOR), 23) ){
                    // It is greater than 23, so get the mod 24 and store it into 
                    //  the hours alarm
                    ToBCD(Mod(THOR, 24), Local1)
                } else {
                    ToBCD(THOR, Local1)
                }
                Store(Local1, THOR)
