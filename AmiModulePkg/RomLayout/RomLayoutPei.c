    ...
    IN VOID *NullPpi)
{
    ...
    for(Area = AmiGetFirstRomArea(); Area != NULL; Area = AmiGetNextRomArea(Area))
    {
        ...
        else if ((((Current.Attributes & AMI_ROM_AREA_FV_ACCESS_ATTRIBUTES) == 0) &&
                (Current.Attributes & AMI_ROM_AREA_SIGNED) == AMI_ROM_AREA_SIGNED  ) &&
                (Current.Type == AmiRomAreaTypeFv))
        {
            Fv = (EFI_FIRMWARE_VOLUME_HEADER*)(UINTN)ReadUnaligned64(&(Current.Address));
            if(Fv->Signature != EFI_FVH_SIGNATURE)
            {
                AMI_ROM_AREA *RomArea = NULL;
                
                // find the ROM AREA address for NVRAM
                RomArea = AmiGetFirstRomAreaByGuid (&gAmiNvramMainRomAreaGuid);
                
                // check addresses and see if it NVRAM.  If it is, don't trigger recovery.
                //  NVRAM tries to detect and deal with recovery on its own.
                if (Area != RomArea)
                {
                    RomArea = NULL;
                    // Now check for NVRAM Backup
