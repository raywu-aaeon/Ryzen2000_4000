...
EFI_STATUS EFIAPI SetIdeDevMode(PCI_DEV_INFO *Dev, BOOLEAN Primary, BOOLEAN Legacy, UINT8 *Override){
    ...
    PCI_CFG_ADDR    addr;
    EFI_STATUS        Status=EFI_SUCCESS;
    UINT8            newpi=0, oldpi=Dev->Class.ProgInterface, tmp=Dev->Class.ProgInterface; 
...
//------------------------------
    ...
    DEBUG((DEBUG_VERBOSE, "\nPciBus: : DeviceAttributes(%d) @ B%X|D%X|F%X Attr=0x%lX; Dev->Attrib=0x%lX; Capab=0x%lX; Operation=%d Status=%r,\n", Set,
    ...
    addr.Addr.Register = PCI_CMD; //command reg
    Status = PciCfg16(Dev->RbIo,addr,FALSE,&testtmp);
    if(EFI_ERROR(Status)) {
       DEBUG((DEBUG_VERBOSE, "\nPciBus: Error in reading CMD register"));
       goto Exit;
    }
      
    DEBUG((DEBUG_VERBOSE, "\nPciBus: : DeviceAttributes() @ B%X|D%X|F%X CMD_REG=0x%lX,\n", 
              Dev->Address.Addr.Bus,Dev->Address.Addr.Device,Dev->Address.Addr.Function, testtmp));
 ...
 /*  Changes done to clear BME if needed to P2P bridges when disable operation is called.
...
//              if (CheckBmeStatus(ext,TempEndPointDev,&ErrorOccured) == FALSE) {
    ...
    Status = PciCfg16(Dev->RbIo,addr,FALSE,&testtmp);
    if(EFI_ERROR(Status)) {
        DEBUG((DEBUG_VERBOSE, "\nPciBus: Error in reading CMD register before exiting the function"));
        goto Exit;
    }
...
//--------------------------------------------
    ...
    DEBUG((DEBUG_VERBOSE, "\nPciBus: : PciIoAttributes() @ B%X|D%X|F%X Attr=0x%lX; Dev->Attrib=0x%lX; Capab=0x%lX; Operation=%d ,\n", 
                            dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function, Attributes, dev->Attrib,capab, Operation));

    if(dev->HostData->VgaDev != NULL){
        DEBUG((DEBUG_VERBOSE, "\nPciBus: Entry : dev->HostData->VgaDev @ B%X|D%X|F%X Attr=0x%lX;  dev->Attrib=0x%lX;\n", 
                                  dev->HostData->VgaDev->Address.Addr.Bus,dev->HostData->VgaDev->Address.Addr.Device,dev->HostData->VgaDev->Address.Addr.Function, Attributes, dev->HostData->VgaDev->Attrib));
    }
    ...
    switch (Operation) {
        ...
        case EfiPciIoAttributeOperationSupported:
            ...
            if(Operation==EfiPciIoAttributeOperationGet)*Result = dev->Attrib;
                ...
                if(dev->HostData->VgaDev != NULL){
                    if(dev != dev->HostData->VgaDev){
                        DEBUG((DEBUG_VERBOSE, "\nPciBus: Get/Supported dev->HostData->VgaDev @ B%X|D%X|F%X Attr=0x%lX; dev->Attrib=0x%lX;\n", 
                                dev->HostData->VgaDev->Address.Addr.Bus,dev->HostData->VgaDev->Address.Addr.Device,dev->HostData->VgaDev->Address.Addr.Function, Attributes, dev->HostData->VgaDev->Attrib));
        ...
        case EfiPciIoAttributeOperationEnable:
            ...
            if((!EFI_ERROR(Status))&& f == FALSE)
            {
                if (dev->HostData->VgaDev==dev) {
                    if ((dev->Attrib & PCI_VGA_ATTRIBUTES) ==0) {
                        DEBUG((DEBUG_VERBOSE, "\nPciBus:  Set/Enable dev->HostData->VgaDev @ B%X|D%X|F%X Attr=0x%lX; dev->Attrib=0x%lX;\n", 
                                                        dev->HostData->VgaDev->Address.Addr.Bus,dev->HostData->VgaDev->Address.Addr.Device,dev->HostData->VgaDev->Address.Addr.Function, Attributes, dev->Attrib));

                        dev->HostData->VgaDev=NULL;
                    }
                }
            }
...
//-----------------------------------
    ...
    if (Pcie2CheckPcie2Compatible (Dev) && Dev->PciExpress->AriData != NULL) { 
...
//-------------------------------------------
    ...
    //------------------    
        ...
        //-----------------------    
            ...
            //PI Spec 1.7 :  Skip device option ROM when 
            //Type-specific flags == 0 and Address Translation Offset == 6.
            //0: Skip device option ROM (do not probe option rom BAR).
            if ((dsc->_TRA == 6) && (dsc->TFlags.TFLAGS == 0)) {
                badbar->BarType=tBarMaxType;
                if (Device->Type == tPciDevice)
                    badbar->BarOffset = PCI_DEV_ROM_BAR ; //0x30 ROM BAR offset;
                else // tPci2PciBrg
                    badbar->BarOffset = PCI_P2P_ROM_BAR; // 0x38
                badbar->IncompType = icNotBar;
            }
  ...
  )
{
    ...
    devaddr.ADDR = Device->Address.ADDR;
    devaddr.Addr.Register = PCI_PRIMARY_STATUS_OFFSET; //Status Register;

    //bit 4 in status register is set if device supports capabilities pointer
    Status = PciCfg16(Device->RbIo, devaddr, FALSE, &sr);
    if (EFI_ERROR(Status))
        return Status;
    if (!(sr & EFI_PCI_STATUS_CAPABILITY))
        return Status; //no reason to continue - capabilities is not supported!
    ...
    if (Device->Type==tPci2CrdBrg) {
        devaddr.Addr.Register = EFI_PCI_CARDBUS_BRIDGE_CAPABILITY_PTR;
    } else  {
        devaddr.Addr.Register = EFI_PCI_CAPABILITY_PTR;
    }
    ...
    //We got something here. Check for alignment and exit if address is not DWORD aligned.
    ...
    while (cp.NextItemPtr && (!(cp.NextItemPtr & (BIT1|BIT0)))) {
        ...
        if (cp.CapabilityID == PCI_CAP_ID_PMI)
            Device->PmiCapOffs = devaddr.Addr.Register;
        ...
        if (cp.CapabilityID == PCI_CAP_ID_PCIEXP) {
            ...
            if (PcieCheckPcieCompatible(Device)) {
                ...
                if ((Device->AmiSdlPciDevData != NULL) && Device->AmiSdlPciDevData->PciDevFlags.Bits.HasSetup) {
                    Status = AmiPciGetPcie1SetupData(
                                            &Device->PciExpress->Pcie1Setup,
                                            Device->AmiSdlPciDevData,
                                            Device->SdlDevIndex, 
                                            FALSE);
                } else {
#endif
...
//#if PCI_X_SUPPORT
        if (cp.CapabilityID == PCI_CAP_ID_PCIX) {
            ...
            Device->PciX = AllocateZeroPool(sizeof(PCIX_DATA));
            ASSERT(Device->PciX);
            if (Device->PciX == NULL)
                return EFI_OUT_OF_RESOURCES;
...
VOID EFIAPI ResetDevicePm(PCI_DEV_INFO *Device)
{
    EFI_STATUS        Status;
    PCI_CFG_ADDR      devaddr;        
    UINT16            pmcsrv; //Turn off the PME assertion and set D0 PM State
    
    if (Device->PmiCapOffs) {
        devaddr.ADDR = Device->Address.ADDR;
        devaddr.Addr.Register = (UINT8)(Device->PmiCapOffs+4);
        Status = PciCfg16(Device->RbIo, devaddr, FALSE, &pmcsrv);
        if (!EFI_ERROR (Status)) {
...
//------------------------------------------
    ...
    if (Device->ParentBrg->DevSetup.Decode4gDisable)
        Device->DevSetup.Decode4gDisable = Device->ParentBrg->DevSetup.Decode4gDisable;
    if (Device->ParentBrg->DevSetup.Pcie1Disable)
        Device->DevSetup.Pcie1Disable = Device->ParentBrg->DevSetup.Pcie1Disable;
    if (Device->ParentBrg->DevSetup.Pcie2Disable)
        Device->DevSetup.Pcie2Disable = Device->ParentBrg->DevSetup.Pcie2Disable;
    if (Device->ParentBrg->DevSetup.HpDisable)
        Device->DevSetup.HpDisable=Device->ParentBrg->DevSetup.HpDisable;
    ...
    //Check Compatibility 
    if (CheckPciCompatibility (Device,NULL,tBarUnused)!=NULL) {
        Device->Incompatible = TRUE;
    } //(EIP41687)
    switch (Device->Type){
        case tPci2PciBrg:
            maxbar=2;
        ...
        case tPci2CrdBrg:
            ...
            break;
        default:
            return EFI_SUCCESS; //other devices not suppose to be examined????
    }
    ...
    for (i=0; i<maxbar; i++) {
        ...
        switch (Device->Bar[i].Type){
            ...
            case tBarMmio64:
                ...
                if (buff) {
                    // SpecialBarCheck :  It could be the BAR that claims - Im a 64 bit BAR but not actually.
                    ...
                    //It could be the BAR that claims - "I'm a 64bit BAR", 
        ...
        if ((Device->Bar[i].DiscoveredType!=tBarUnused) && (Device->Bar[i].Type == tBarUnused)) {
            oldv = 0;
        }
...
//--------------------------
    ...
    DEBUG((DEBUG_VERBOSE,"BAR: Bas=0x%lX; Len=0x%lX;\tGra=0x%lX;\tOfs=0x%X;Owner->[B%X|D%X|F%X] DEV_TYPE=%d;\n",
        Bar->Base, Bar->Length, Bar->Gran, (Bar->ExtOffset == 0) ? Bar->Offset : Bar->ExtOffset,
                Bar->Owner->Address.Addr.Bus, Bar->Owner->Address.Addr.Device, 
                Bar->Owner->Address.Addr.Function, Bar->Owner->Type));
...
OptimizeBrgResource (
  IN  PCI_DEV_INFO      *Brg,
  IN  MRES_TYPE         ResType)
{
    EFI_STATUS          Status = 0;
//    PCI_DEV_INFO        *dev; //Device Data used for iteration,
            ...
            if (bar->Gran >= balign->MaxGran) {
                ...
                if (ebar->Length) {
                    ...
                    //-----------------------
                        ...
                        val[0] = ebar->Gran+1;
                        Status = DbeLocateKey(&DevDb, 0, val, &p, &Vicinity, &Index);
                        ...
                        Status = DbeGoToIndex (&DevDb, 0, Index, (VOID**)&bar); 
                        if (EFI_ERROR(Status)){
                            if (!((Status == EFI_DBE_EOF) || (Status == EFI_DBE_BOF))){
                                ASSERT_EFI_ERROR (Status);
                                goto ExitLbl;
                        ...
                        if ((Vicinity == -1) && (bar != NULL)) {
                            ...
                            if (Index > 0) { 
                                bar = NULL;
                                Status = DbeGoToIndex (&DevDb, 0, (Index - 1), (VOID**)&bar); 
                                if (EFI_ERROR(Status)) {
                                    if (!((Status==EFI_DBE_EOF) || (Status==EFI_DBE_BOF)))  {
                                        ASSERT_EFI_ERROR (Status);
                                        goto ExitLbl;
...
EFI_STATUS EFIAPI EnumerateBus(PCI_DEV_INFO *ParentBrg, BOOLEAN BusReScan)
{
	...
	UINT8           ari_device_is_next_function_Number = 0 ;
...
//----------------------------------    
    ...
    if (ari_device_is_next_function_zero) continue; 
        ...
        for(pcidev.Addr.Function=0;  pcidev.Addr.Function<=PCI_MAX_FUNC; pcidev.Addr.Function++){
            ...
            if (!(EFI_ERROR(Status)) && PcieCrsSupport){
            ...
            if(id == 0xFFFFFFFF || id==0){
                DEBUG((DEBUG_INFO,"...%r\n",Status));
                 if (!mf) {
                    if (ari && (ari_device_is_next_function_zero == FALSE)) {
                        // Check Previous device Function points to the current function. If not,matching continue the function loop.
                        // If matched, then break function loop and Device loop immediately as the device is not present.
                        if ((ari_device_is_next_function_Number / (PCI_MAX_FUNC+1)) == pcidev.Addr.Device && (ari_device_is_next_function_Number % (PCI_MAX_FUNC +1)) == pcidev.Addr.Function) {
                            ari_device_is_next_function_zero = TRUE;
                            break;
                        } else {
                           continue; // ARI Device with F8 as zero (D1 F0)
                        }
                    } else {
                        break; // Device not exists, break...
                    }
                 } else {
                    continue;  //if device was identified as multifunc keep scanning
                 }
            }    
                ...
                case HDR_TYPE_DEVICE    : 
...
#endif
                    {
                        if(dev->SkipDevice){
                            ...
                            if (ari) {
                                DEBUG((DEBUG_INFO,"PciBus: PCI Device @ [B%X|D%X|F%X]...", 
                                      dev->Address.Addr.Bus, dev->Address.Addr.Device, dev->Address.Addr.Function));
                                DEBUG((DEBUG_INFO,"dev->PciExpress->AriData->AriCaps.NextFuncN = 0x%X\n",dev->PciExpress->AriData->AriCaps.NextFuncN));
                                ari_device_is_next_function_Number = (UINT8)dev->PciExpress->AriData->AriCaps.NextFuncN;
                              if (dev->PciExpress->AriData->AriCaps.NextFuncN == 0)  {
                                ari_device_is_next_function_zero = TRUE;
                              }
                            }
  ...
  )
{
    ...
    if (dev->AmiSdlPciDevData != NULL) 
        virt = dev->AmiSdlPciDevData->PciDevFlags.Bits.Virtual;
    ...
    switch (ResType) {
        case tBarIo:
            ...
            wt = tBarIo32;
            p2pct = rcOneOf;
            if (Brg->Common.Type == tPci2CrdBrg)
                p2pct = rcBoth;
            break;
        ...
        default: 
            return; //no other combinations supported
    }//switch
...
//#endif
        ...
        if ((dev->Type == tPci2PciBrg) || (dev->Type == tPci2CrdBrg)) {
            if (cnv) {
                ...
                ConvertResources((PCI_BRG_INFO*)dev, ResType, p2pct, CombineMemPmem);
            }
        }
    } //Child loop
    //we did it!!!
...
//#endif
            ...
            PcdSetBoolS (AmiPcdPciOutOfResourcesStatus, gPciBusOutOfRes);
            ...
            if (gBusResourceHit && gPciBusOutOfRes)
            {
                ...
                else
                {
                    ...
                    PcdSetBoolS (AmiPcdPciOutOfResourcesStatus, FALSE);
                    continue;
                }
            }
            if (gBusResourceHit)
...
ExitLbl:
    ...
    if((Status==EFI_NOT_FOUND)&&(ExitStatus==EFI_SUCCESS)){
        ...
        if(Dev->Incompatible) {
            ...
            bbp = CheckPciCompatibility(rombar->Owner,rombar,rombar->Type);
            if(!bbp) bbp=CheckPciCompatibility(rombar->Owner,rombar,tBarMaxType);
            //this function will be called only if compatibility issue exists with the device
            //so bbp must be valid so don'd load the option rom embedded in BIOS
            if(bbp) {
                return EFI_NOT_FOUND; 
            }
        }
    ...
    for(j=0; j<gHostCnt; j++){
        ...
        for(i=0; i<lhst->RbCount; i++){
            ...
            if(rbrg->Handle==ControllerHandle){ 
                ...
                // Install EfiPciEnumerationCompleteProtocolGuid in each Host bridge handle
                // [As per PI spec, installed only once for each HB after EFI_PCI_IO_PROTOCOL and EFI_DEVICE_PATH_PROTOCOL are installed]
                if ((lhst->EnumerationCompleteFlag == FALSE) && (i == (lhst->RbCount-1))) {
