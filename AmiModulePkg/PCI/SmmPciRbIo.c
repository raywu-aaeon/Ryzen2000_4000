...
//--------------------------------------------
  ...
  if (Delay == 0) return EFI_SUCCESS;
    ...
    // Assume a 30 uS delay (300, 100ns units) for each poll iteration
...
/**
   ...
   The Mem.Read(), and Mem.Write() functions enables a driver to access PCI controller
  ...
  )
{
    ...
    //Generic Function never checks first parameter set it to NUll.
...
/*
    Initialize the SMM RB IO driver internal Data.
    
    @param  Nothing
    
    @retval EFI_STATUS
    @retval EFI_SUCCESS            SmmRbIoData Initialized successfully.
    @retval EFI_INVALID_PARAMETER  Width is invalid for this PCI root bridge.
    @retval EFI_INVALID_PARAMETER  Buffer is NULL.
    @retval EFI_OUT_OF_RESOURCES   The request could not be completed due to a lack of resources.
 */
...
//--------------------------------
    ...
    //Get some SMM Memory to accommodate private data structures...
    ...
    for(j=0,k=0; j<gPciHandoffProtocol->HostCnt;j++){
        ...
        for(i=0;i<host->RootBridgeCount;i++){
            ...
            //if DXE RB has resources at that point - copy it here
            /*
...
/*
    Installs Pci RootBridge IO Protocol.
    
    @param[in]  ImageHandle        The image handle of the driver.
    @param[in]  SystemTable        The system table.
    
    @retval EFI_STATUS
    @retval EFI_SUCCESS            SmmPciRootBridgeIoProtocol installed successfully.
    @retval EFI_INVALID_PARAMETER  Width is invalid for this PCI root bridge.
    @retval EFI_INVALID_PARAMETER  Buffer is NULL.
    @retval EFI_OUT_OF_RESOURCES   The request could not be completed due to a lack of resources.
...
//--------------------------
    ...
    //Instance must be there since this driver has dependency on its protocol
    if(EFI_ERROR(Status)){
        ASSERT_EFI_ERROR (Status);
        return EFI_UNSUPPORTED;
    }
    ...
    //Install SmmRbIo protocol instances...
