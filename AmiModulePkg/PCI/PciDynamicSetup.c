...
/**
    This function generates PCI device title string and Help string
...
//--------------------------------
    ...
    if (PciDevGotoData->PciSdlInfo->PciDevFlags.Bits.OnBoard){
        ...
        UnicodeSPrint (s, StrSize, L"%s  %s [%s]", locationstr, statusstr, classstr);
    } else {
        ...
        if(PciDevGotoData->PciDevInfo==NULL){
            ...
            if(PciDevGotoData->ParentBrgInfo==NULL){
                ...
                UnicodeSPrint (s, StrSize, L"%s #%2d %s [FROM BRG (%s)]",
                    locationstr, PciDevGotoData->PciSdlInfo->Slot, statusstr, classstr);
            } else {
                UnicodeSPrint (s, StrSize, L"%s #%2d %s [FROM BRG (B%X|D%X|F%X)]",
            ...
            UnicodeSPrint (s, StrSize, L"%s #%2d %s [%s]", locationstr, PciDevGotoData->PciSdlInfo->Slot, statusstr, classstr);
        }
    }
...
/**
    Returns associated with passed PCI Volatile or Non Volatile varstoreid.
...
VOID
EFIAPI
PciSetupCreatePciLatencyOptions (
   VOID *OpcodeHandle 
 )
{
    ...
    UINT8            flags;
//--------------------------------
    for (Index = 0, val = 32; Index < (sizeof(pcilatstr)/sizeof(EFI_STRING_ID)); Index++, val+=32){
        
         flags = 0; 
         if ((OpcodeHandle == gPciDevForm.PciLatencyOpCodeHandle) && (Index == 0))
             flags = (EFI_IFR_OPTION_DEFAULT | EFI_IFR_OPTION_DEFAULT_MFG);
         
         if ((OpcodeHandle == gPciDevForm.PciXLatencyOpCodeHandle) && (Index == 1))
             flags = (EFI_IFR_OPTION_DEFAULT | EFI_IFR_OPTION_DEFAULT_MFG);
...
VOID
EFIAPI 
PciSetupCreatePcie1MaxRrOptions(
  VOID                  *OpcodeHandle,
  PCI_DEV_INFO          *dev
)
{
    ...
    static EFI_STRING_ID    Mrrstr[]={
                                      ...
                                      STRING_TOKEN(STR_MP512), STRING_TOKEN(STR_MP1024), 
                                      STRING_TOKEN(STR_MP2048), STRING_TOKEN(STR_MP4096)
                                     };
...
//--------------------------------
    ...
    if ((gPciDevForm.MaxPayLoadOpCodeHandle == OpcodeHandle) && (SHOW_MAX_PAYLOAD_SIZE_BASED_ON_ROOT_PORT_CAP) ) { 
        while(dev) {
           if (dev->PciExpress != NULL) {
               if (dev->PciExpress->PcieCap.PortType == ROOT_PORT_TYPE) {
                   break;
               }
           }
           dev = dev->ParentBrg;
        ...
        if (dev != NULL) {
            switch (dev->PciExpress->DeviceCap.MaxPayload) {
                case 0 :
                    Count = 1; // 128b
                    break;
                case 1 :
                ...
                case 5 :
                    ...
                    break;
                default : 
                    Count = (sizeof(Mrrstr)/sizeof(EFI_STRING_ID));
           }
        ...
        for (Index = 0; Index < (sizeof(Mrrstr)/sizeof(EFI_STRING_ID)); Index++){
             HiiCreateOneOfOptionOpCode (
                           OpcodeHandle,
                           Mrrstr[Index],
                           0,
...
//--------------------------------
    ...
    switch(OpType){
        ...
        case optIo:
            ...
            PaddingResource = PaddingResource / 0x1000;  
            {
                UINT8 Index1 = 0;
...
{
    EFI_STRING                StringBuffer = NULL;
    EFI_STRING                FeatureStringBuffer = NULL;
//-------------------------    
    //1. Empty Slot here with disabled bridge... 
    if (GotoData->PciDevInfo == NULL && GotoData->ParentBrgInfo == NULL) {
        StringBuffer = HiiGetString(
                             gPciHiiHandle,
                             STRING_TOKEN(STR_DEV_INFO_EMPTY_STR1),
                             NULL); 
        HiiSetString(
              gPciHiiHandle,
              STRING_TOKEN(STR_PCI_DEV_SUBTITLE1),
              StringBuffer, 
              NULL);
        ...
        StringBuffer = HiiGetString(
                             gPciHiiHandle,
                             STRING_TOKEN(STR_DEV_INFO_EMPTY_STR2),
                             NULL);
        HiiSetString(
              gPciHiiHandle,
              STRING_TOKEN(STR_PCI_DEV_SUBTITLE2),
              StringBuffer,
              NULL);
    }
    ...
    if ((GotoData->PciDevInfo != NULL) || (GotoData->ParentBrgInfo != NULL)) {
        EFI_STRING            onx = L"X";
        EFI_STRING            off = L" ";
        EFI_STRING            Gen1String, Gen2String, AriString, HotPlugString;
        UINTN                 StrSize = 200;
    //----------------------------
        StringBuffer = AllocateZeroPool (StrSize);
        if (StringBuffer == NULL) {
            return;
        }

        FeatureStringBuffer = HiiGetString (
                                    gPciHiiHandle,
                                    STRING_TOKEN(STR_DEV_INFO_STR1),
                                    NULL);
        ...
        //----------------------
            UnicodeSPrint (
                   ...
                   StrSize,
                   FeatureStringBuffer,
                   GotoData->ParentBrgInfo->RbIo->SegmentNumber,
                   Brg->Res[rtBus].Base, 
                   GotoData->PciSdlInfo->Device,
                   GotoData->PciSdlInfo->Function,
                   0xFFFF,
                   0xFFFF);
        } else {
            ...
            UnicodeSPrint (
                   ...
                   StrSize,
                   FeatureStringBuffer,
                   GotoData->PciDevInfo->RbIo->SegmentNumber,  
                   GotoData->PciDevInfo->Address.Addr.Bus, 
                   GotoData->PciDevInfo->Address.Addr.Device,
                   GotoData->PciDevInfo->Address.Addr.Function,
                   GotoData->PciDevInfo->DevVenId.VenId,
                   GotoData->PciDevInfo->DevVenId.DevId);
        ...
        if (FeatureStringBuffer != NULL) {
            FreePool (FeatureStringBuffer);
        }

        ZeroMem (StringBuffer, StrSize);
        ...
        FeatureStringBuffer = HiiGetString (
                                    gPciHiiHandle,
                                    STRING_TOKEN(STR_DEV_INFO_STR2),
                                    NULL);
        UnicodeSPrint (
                StringBuffer,
                StrSize,
...
//    gPciDevForm.Mmio64OpCodeHandle = HiiAllocateOpCodeHandle ();
    ...
    PciSetupCreatePaddingOptions (
                  gPciDevForm.BusPaddOpCodeHandle,
                  optBus,
                  GotoData->ParentBrgInfo->AmiSdlPciDevData->ResourcePadding[0]
                  );
    ...
    PciSetupCreatePaddingOptions (
                  gPciDevForm.IoPaddOpCodeHandle,
                  optIo,
                  GotoData->ParentBrgInfo->AmiSdlPciDevData->ResourcePadding[1]
                  );
    ...
    PciSetupCreatePaddingOptions (
                  gPciDevForm.Mmio32OpCodeHandle,
                  optMmio,
                  GotoData->ParentBrgInfo->AmiSdlPciDevData->ResourcePadding[3]
                  );
    ...
    PciSetupCreatePaddingOptions (
                  gPciDevForm.Mmio32PfOpCodeHandle,
                  optMmio,
                  GotoData->ParentBrgInfo->AmiSdlPciDevData->ResourcePadding[4]
                  );
...
*/    
    ...
    PciSetupCreatePaddingOptions (
                  gPciDevForm.Mmio64PfOpCodeHandle,
                  optMmio64,
                  GotoData->ParentBrgInfo->AmiSdlPciDevData->ResourcePadding[6]
                  );
...
//----------------------------    
    ...
    if (!DontTouch) {
        PciSetupCreatePcie1MaxRrOptions (gPciDevForm.MaxPayLoadOpCodeHandle, GotoData->PciDevInfo);
    ...
    if (!DontTouch){
        PciSetupCreatePcie1MaxRrOptions (gPciDevForm.MaxReadReqOpCodeHandle, GotoData->PciDevInfo);
        //Now create OneOf OpCode...
        HiiCreateOneOfOpCode (
            gPciDevForm.Pcie1StartOpCodeHandle,                //*OpCodeHandle
    ...
    if (!gIntegratedPcie){
        ...
        // check the root port capability for ASPM support.
        if (!DontTouch && SHOW_ASPM_OPTION_BASED_ON_ROOT_PORT_CAP) {
            dev = GotoData->PciDevInfo;
            while (dev) {
                if (dev->PciExpress != NULL) {
                    if (dev->PciExpress->PcieCap.PortType == ROOT_PORT_TYPE) {
                        break;
            ...
            if ((dev != NULL) && (dev->PciExpress->LinkCap.AspmSupport == 0x0)) {
                AspmSupport = FALSE;
            }
        }
        if (!DontTouch && AspmSupport ) {
            HiiCreateOneOfOptionOpCode (
        ...
        if (!DontTouch && AspmSupport) {
            ...
            HiiCreateSubTitleOpCode (gPciDevForm.Pcie1StartOpCodeHandle,
                STRING_TOKEN(STR_PCIE_ASPM_WARN),
                STRING_TOKEN(STR_EMPTY),
                0, 0);
            HiiCreateSubTitleOpCode (gPciDevForm.Pcie1StartOpCodeHandle,
                STRING_TOKEN(STR_PCIE_ASPM_WARN1),
                STRING_TOKEN(STR_EMPTY),
                0, 0);
        }
...
//--------------------------------------------
    ...
    if(gotodata->PciDevInfo!=NULL){
        ...
        if(pcie1 && ((gotodata->PciDevInfo)->PciExpress->PcieCap.PortType==PCIE_TYPE_RC_INTEGR_EP || 
                (gotodata->PciDevInfo)->PciExpress->PcieCap.PortType==PCIE_TYPE_RC_EVT_COLLECT)) gIntegratedPcie = TRUE;
    } else {
...
/**
    Prepares the IFR labels within the PCI page for the controls insertion. It
    also gets the list of PciDev handles.
    ...
    @note  
  This call does not free the memory allocated earlier for PCI_IFR_INFO;
...
//DEBUG//DEBUG//DEBUG//DEBUG//DEBUG//DEBUG
    ...
    //----------------------------------
        ...
        for (j=0; j<PciDevHandleCount; j++){
            ...
            if (!sdlpcidev->PciDevFlags.Bits.OnBoard && !IsFunc0(dev)) 
                continue;
            ...
            if (dev->SdlDevCount==1) {
                if (sdlpcidev == dev->AmiSdlPciDevData) {
                    ...
                    (PciIfrInfo->PciDevSetupData[i])->ParentBrgInfo = dev->ParentBrg;
                    break;
                }
            ...
            //--------------    
                ...
                //If the Device and Parent Bridge are initialized break the loop. //for (j=0; j<PciDevHandleCount; j++)
                if (((PciIfrInfo->PciDevSetupData[i])->PciDevInfo != NULL) && 
                    ((PciIfrInfo->PciDevSetupData[i])->ParentBrgInfo != NULL))
                    break;
            }
        }
...
/**
    This will update the PCI setup Default values.
...
)
{
    ...
    UINTN                      Index = (UINTN)(KeyValue & (~PCI_GOTO_ID_BASE));
    PCI_DEV_SETUP_GOTO_DATA    *gotodata;
    EFI_STATUS                 Status = EFI_SUCCESS;
    UINTN                      Size;

    gotodata = gPciIfrInfo.PciDevSetupData[Index];

    if ((gotodata == NULL) || (KeyValue>=PCIE1_GOTO_ID))
        return;

    gotodata->PciDev.Decode4gDisable = !(ABOVE_4G_PCI_DECODE_DEFAULT_VALUE);
    gotodata->PciDev.PciLatency = 32;
    gotodata->PciDev.PciXLatency = 64;
    gotodata->PciDev.VgaPallete = VGA_PALLETE_SNOOP_SETUP_DEFAULT_VALUE;
    gotodata->PciDev.PerrEnable = PERR_SETUP_DEFAULT_VALUE;
    gotodata->PciDev.SerrEnable = SERR_SETUP_DEFAULT_VALUE;
    gotodata->PciDev.Pcie1Disable = PCIE_GEN1_ENABLE_SETUP_DEFAULT_VALUE;
    gotodata->PciDev.Pcie2Disable = PCIE_GEN2_ENABLE_SETUP_DEFAULT_VALUE;
    gotodata->PciDev.HpDisable = !(HOTPLUG_SETUP_DEFAULT_VALUE);

    Size = sizeof(PCI_DEVICE_SETUP_DATA);
    Status = HiiLibSetBrowserData (
                   Size,
                   &gotodata->PciDev, 
                   &PciFormsetGuid,
                   gotodata->NvPciDevName);
    DEBUG((DEBUG_VERBOSE,"PciSetup: PCI Dev HiiLibSetBrowserData Status = %r\n", Status));

    if (gotodata->Pcie1Support) {

        gotodata->Pcie1Dev.RelaxedOrdering = RELAXED_ORDERING_SETUP_DEFAULT_VALUE;
        gotodata->Pcie1Dev.ExtTagField = EXTENDED_TAG_FIELD_SETUP_DEFAULT_VALUE;
        gotodata->Pcie1Dev.NoSnoop = NO_SNOOP_SETUP_DEFAULT_VALUE;
        gotodata->Pcie1Dev.ExtendedSynch = EXTENDED_SYNC_SETUP_DEFAULT_VALUE;
        gotodata->Pcie1Dev.ClockPm = CLOCK_PM_SETUP_DEFAULT_VALUE;
        gotodata->Pcie1Dev.LnkDisable = PCIE_LINK_ENABLE_SETUP_DEFAULT_VALUE;
        gotodata->Pcie1Dev.MaxPayload = PCI_SETUP_AUTO_VALUE;
        gotodata->Pcie1Dev.MaxReadRequest = PCI_SETUP_AUTO_VALUE;
        gotodata->Pcie1Dev.LnkTrRetry = 5;
        gotodata->Pcie1Dev.LnkTrTimeout = 1000;
        gotodata->Pcie1Dev.AspmMode = 0;

        Size = sizeof(PCIE1_SETUP_DATA);
        Status = HiiLibSetBrowserData (
                       Size,
                       &gotodata->Pcie1Dev, 
                       &PciFormsetGuid,
                       gotodata->NvPcie1DevName);
        DEBUG ((DEBUG_VERBOSE,"PciSetup: PCIE1 HiiLibSetBrowserData Status = %r\n", Status));
    }

    if (gotodata->Pcie2Support) {

        gotodata->Pcie2Dev.LnkSpeed = PCI_SETUP_AUTO_VALUE;
        gotodata->Pcie2Dev.AriFwd = ARI_FORWARDING_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.AtomOpReq = ATOMIC_OPERATION_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.AtomOpEgressBlk = ATOMIC_OPERATION_EGRESS_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.IDOReq = IDO_REQ_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.IDOCompl = IDO_COMPLETION_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.LtrReport = LTR_REPORTING_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.E2ETlpPrBlk = E2E_TLP_PREFIX_BLOCK_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.ComplSos = COMPLIANCE_SOS_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.HwAutoWidth = HW_AUTO_WIDTH_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.HwAutoSpeed = HW_AUTO_SPEED_SETUP_DEFAULT_VALUE;
        gotodata->Pcie2Dev.ComplTimeOut = 0xFF;

        Size = sizeof(PCIE2_SETUP_DATA);
        Status = HiiLibSetBrowserData (
                       Size,
                       &gotodata->Pcie2Dev, 
                       &PciFormsetGuid,
                       gotodata->NvPcie2DevName);
        DEBUG((DEBUG_VERBOSE,"PciSetup: PCIE2 HiiLibSetBrowserData Status = %r\n", Status));
    }

    if (gotodata->HpSupport) {

        pBS->CopyMem(&gotodata->PcieHpDev,&gotodata->ParentBrgInfo->AmiSdlPciDevData->ResourcePadding, sizeof(PCI_HP_SETUP_DATA));
        Size = sizeof(PCI_HP_SETUP_DATA);
        Status = HiiLibSetBrowserData (
                       Size,
                       &gotodata->PcieHpDev, 
                       &PciFormsetGuid,
                       gotodata->NvPcieHpDevName);
        DEBUG((DEBUG_VERBOSE,"PciSetup: PCI_HP HiiLibSetBrowserData Status = %r\n", Status));
    }
}

/**
    This will collect the Nvram value for PCI setup variable.
...
)
{
    ...
    PCI_DEV_SETUP_GOTO_DATA   *gotodata;
    UINTN                     Size;
    BOOLEAN                   Pcie1, Pcie2, Hp;

    gotodata = gPciIfrInfo.PciDevSetupData[Index]; 

    Size = sizeof(PCI_DEVICE_SETUP_DATA);
    gotodata->NvPciDevName = AmiPciGetPciVarName (
                                   gotodata->PciSdlInfo,
                                   gotodata->PciDevIdx, 
                                   dtDevice);
    pRS->GetVariable (
    ...
    // AmiPciGetPcie1SetupData
    if (Pcie1) {
        gotodata->Pcie1Support = TRUE;

        Size = sizeof(PCIE1_SETUP_DATA);
        gotodata->NvPcie1DevName = AmiPciGetPciVarName (
                                         gotodata->PciSdlInfo,
                                         gotodata->PciDevIdx,
                                         dtPcie1);
        pRS->GetVariable (
    ...
    // AmiPciGetPcie2SetupData
    if (Pcie2) {
        gotodata->Pcie2Support = TRUE;
        Size = sizeof(PCIE2_SETUP_DATA);
        gotodata->NvPcie2DevName = AmiPciGetPciVarName (
                                         gotodata->PciSdlInfo,
                                         gotodata->PciDevIdx, 
                                         dtPcie2);
        pRS->GetVariable (
    ...
    if (Hp) {
        gotodata->HpSupport = TRUE;
        Size = sizeof(PCI_HP_SETUP_DATA);
        gotodata->NvPcieHpDevName = AmiPciGetPciVarName (
                                          gotodata->PciSdlInfo,
                                          gotodata->PciDevIdx,
                                          dtHp);
        pRS->GetVariable (
...
VOID EFIAPI InitPciIfr(IN EFI_EVENT Event, IN VOID *Context)
{
    EFI_STATUS            Status;
//------------------------------------
    DEBUG((DEBUG_VERBOSE, "PciSetup: InitPciIfr(Event 0x%X, Context @0x%X) = ",Event, Context));
...
//-----------------------------    
    ...
    if (pd == NULL) pd = &d;
    UnicodeSPrint (s, sizeof(s), L"%s%02X.%02d.%02d", pd, PCI_BUS_MAJOR_VER, PCI_BUS_MINOR_VER, PCI_BUS_REVISION);
...
//    EFI_DEADLOOP();
    while(IfrData->PciDevCount){
        ...
        //-----------------    
            if (gt->PciDevInfo == NULL) continue;
