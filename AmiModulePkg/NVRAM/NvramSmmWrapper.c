...
EFI_STATUS EFIAPI ActivateSmmWrapper(){
    ...
    Status = pBS->LocateProtocol ( &gEfiSmmCommunicationProtocolGuid, NULL, (VOID**)&SmmCommProtocol );
...
{
    pRS->ConvertPointer (0, (VOID**)&NvramSmmCommunicationBuffer);
...
EFI_STATUS NvramSmiDxeEntry()
{
    ...
    Status = pBS->AllocatePool ( EfiRuntimeServicesData, NvramSmmCommunicationBufferSize, (VOID**)&NvramSmmCommunicationBuffer );
    ASSERT_EFI_ERROR(Status);
    if (EFI_ERROR(Status)) return Status;
