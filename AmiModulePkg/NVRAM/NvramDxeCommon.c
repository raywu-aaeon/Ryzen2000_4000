...
EFI_STATUS CopyBootOptions(
    ...
    for(i=0; i<BootOrderSize/sizeof(UINT16); i++){
        ...
        UINTN PrintSize;
        PrintSize = UnicodeSPrint(BootVariableName,sizeof(BootVariableName),L"Boot%04X",BootOrder[i]);
        // PrintSize - Unicode characters in the produced output buffer not including the Null-terminator.
        if (PrintSize != (sizeof(BootVariableName)/sizeof(BootVariableName[0]) - 1))
            return EFI_OUT_OF_RESOURCES;
...
EFI_STATUS EFIAPI AmiNvramUpdateProtocolMigrateNvram(
    ...
    if (NvramBackupAddress != 0 && !(NvramBackupAddress >= gDescriptor.BaseAddress && NvramBackupAddress + NvramSize <= gDescriptor.BaseAddress + gDescriptor.Length))
        return EFI_SECURITY_VIOLATION;
