...
#if (defined(POWERUP_IN_STANDBY_SUPPORT) && (POWERUP_IN_STANDBY_SUPPORT == 1))
    TRUE,         // PowerUpInStandby feature is supported or not
#else
    FALSE,
#endif
    
#if (defined(POWERUP_IN_STANDBY_MODE) && (POWERUP_IN_STANDBY_MODE == 1))
    TRUE,            // PowerUpInStandby mode
#else
    FALSE,
#endif
#if (defined(DiPM_SUPPORT) && (DiPM_SUPPORT == 1))
    TRUE,                        // Device Initiated power management
#else
    FALSE,
#endif
#if (defined(ENABLE_DIPM) && (ENABLE_DIPM == 1))
    TRUE,
#else
    FALSE,
#endif
#if (defined(DEVICE_SLEEP_SUPPORT) && (DEVICE_SLEEP_SUPPORT == 1))
    TRUE,
#else
    FALSE,
#endif
#if (defined(ENABLE_DEVICE_SLEEP) && (ENABLE_DEVICE_SLEEP == 1))
    TRUE,
#else
    FALSE,
#endif
#if (defined(USE_PCIIO_MAP_ADDRESS_FOR_DATA_TRANSFER) && (USE_PCIIO_MAP_ADDRESS_FOR_DATA_TRANSFER == 1))
    TRUE
#else
    FALSE
#endif
...
)
{
    ...
    if ( HddSecurityInitProtocol == NULL) {
        Status = pBS->LocateProtocol (&gAmiHddSecurityInitProtocolGuid,
                                      NULL,
                                      (VOID **) &HddSecurityInitProtocol);
        if (EFI_ERROR(Status)) {
            DEBUG((DEBUG_INFO,"\n Failed to Locate gAmiHddSecurityInitProtocolGuid: %r",Status));
        }
    }
    ...
    if (OpalSecInitProtocol == NULL) {
        ...
        if (EFI_ERROR(Status)) {
            DEBUG((DEBUG_INFO,"\n Failed to Locate gAmiHddOpalSecInitProtocolGuid: %r",Status));
        }
    }
    ...
    if (HddSmartInitProtocol == NULL) {
        ...
        if (EFI_ERROR(Status)) {
            DEBUG((DEBUG_INFO,"\n Failed to Locate gAmiHddSmartInitProtocolGuid: %r",Status));
        }
    }

#if defined(ACOUSTIC_MANAGEMENT_DRIVER_SUPPORT) && (ACOUSTIC_MANAGEMENT_DRIVER_SUPPORT != 0)
    if (gHddAcousticInitProtocol == NULL) {
        Status = pBS->LocateProtocol (&gHddAcousticInitProtocolGuid,
                                   NULL,
                                   (VOID **) &gHddAcousticInitProtocol);
        if (EFI_ERROR(Status)) {
            DEBUG((DEBUG_INFO,"\n Failed to Locate gHddAcousticInitProtocolGuid: %r",Status));
        }
    }
#endif
    ...
    for (dlink = AhciBusInterface->SataDeviceList.pHead; dlink; dlink = dlink->pNext) {
        ...
        Status = pBS->HandleProtocol( SataDevInterface->AhciBusInterface->ControllerHandle, 
                                      &gEfiAtaPassThruProtocolGuid, 
                                      (VOID**)&(SataDevInterface->AtaPassThru) );
        if(EFI_ERROR(Status)){
            DEBUG((DEBUG_ERROR,"AHCI: Unable to locate AtaPassThru protocol : %r\n", Status));
        }
        ...
        if(HddSecurityInitProtocol != NULL) {
            ...
            if(EFI_ERROR(Status)){
                DEBUG((DEBUG_INFO,"\n Unable to Install HDDSecurity Interface: %r",Status));
            }
            if(TempHddSecurityProtocolPtr == NULL) {
                ...
                if (EFI_ERROR(Status)) {
                    DEBUG((DEBUG_INFO,"\n HddSecurityProtocol is not supported by the device: %r",Status));
                }
            }
        }
        ...
        if (EFI_ERROR(Status)) {
            ...
            if(TcgStorageSecurityInitProtocol == NULL) {
                ...
                if (EFI_ERROR(Status)) {
                    DEBUG((DEBUG_INFO,"\n TcgStorageSecurityProtocol is not supported by the device: %r",Status));
                }
            }
 ...
 )
{
    ...
    if (NumberOfChildren) {
        ...
        Status = pBS->LocateProtocol (&gAmiScsiPassThruInitProtocolGuid,
         ...
         if ( AhciBusInterface->FisMapping != NULL ) {
             // Unmap the mapped memory for the Received FIS
             AhciBusInterface->PciIO->Unmap( AhciBusInterface->PciIO,
                                             AhciBusInterface->FisMapping);
         }
         
         // Free the Pages allocated for the FIS and Command List
         if (AhciBusInterface->PortFISBaseAddress) {
             Status = pBS->FreePages(AhciBusInterface->PortFISBaseAddress,
                            EFI_SIZE_TO_PAGES(AhciBusInterface->NumberofPortsImplemented * RECEIVED_FIS_SIZE + 0x100 ));
             if (EFI_ERROR(Status)) {
                 return Status;
             }
         }
         if ( AhciBusInterface->CommandListMapping != NULL ) {
             // Unmap the mapped memory for the CommandList Address
             AhciBusInterface->PciIO->Unmap( AhciBusInterface->PciIO,
                                             AhciBusInterface->CommandListMapping);
         }
         
         if (AhciBusInterface->PortCommandListBaseAddress) {
             Status = pBS->FreePages((AhciBusInterface->PortCommandListBaseAddress),EFI_SIZE_TO_PAGES(COMMAND_LIST_SIZE_PORT * 2));
             if (EFI_ERROR(Status)) {
                  return Status;
              }
         }
  ...
  )
{
    ...
    // Calculate Received FIS size for the implemented ports + additional 0x100 bytes for alignment
    ...
    AhciBusInterface->PortFISBaseAddrEnd = FisReceiveBaseAddr + ReceiveFisSize;
    
    AhciBusInterface->PortFISBaseAddr       = (FisReceiveBaseAddr & (~0xFF))+ 0x100; 
    AhciBusInterface->PortFISMappedBaseAddr    = (FisReceiveMappedBaseAddr & (~0xFF))+ 0x100;
    
    // Allocate memory for Command List (1KB aligned) and Command Table (128KB aligned).
    // All the ports in the controller will share Command List and Command table data Area.
    
    AhciBusInterface->PortCommandListBaseAddr  = (CommandListBaseAddr & (~0x3FF)) + 0x400;
    AhciBusInterface->PortCommandListMappedBaseAddr  = (CommandListMappedBaseAddr & (~0x3FF)) + 0x400;
    ...
    AhciBusInterface->PortCommandTableMappedBaseAddr = AhciBusInterface->PortCommandListMappedBaseAddr + 0x80;
...
)
{
    ...
    UINT8         Index, PortNumber;
    UINT32        Data32;
    UINT64          Supports=0;
    ASLR_QWORD_ASD  *Resources=NULL;
    SATA_DEVICE_INTERFACE    TempSataDevInterface;

    // Make sure AHCI Base address is programmed Properly
    Status = AhciBusInterface->PciIO->GetBarAttributes (
                                                AhciBusInterface->PciIO,
                                                PCI_ABAR_INDEX,
                                                &Supports,
                                                (VOID**)&Resources);

    if (EFI_ERROR(Status) || (!Resources) || (Resources->Hdr.Name==ASLV_SR_EndTag)) { 
        if (Resources)
    ...
    if (!AhciBusInterface->AhciBaseAddress) 
        return EFI_DEVICE_ERROR;   
    ...
    PortsImplemented = AhciBusInterface->HBAPortImplemented;
    PortNumber = 0;
    for (Index=0; PortsImplemented; PortsImplemented>>=1, PortNumber++){
        if (PortsImplemented & 1) {
            // Program PxCLB and PxFB
            HBA_PORT_WRITE_REG64 (AhciBusInterface,
                                  PortNumber,
                                  HBA_PORTS_CLB,
                                  AhciBusInterface->PortCommandListMappedBaseAddr);

            HBA_PORT_WRITE_REG64 (AhciBusInterface,
                                  PortNumber,
                                  HBA_PORTS_FB,
                                  AhciBusInterface->PortFISMappedBaseAddr +(Index * RECEIVED_FIS_SIZE));
            Index++;
            // Clear Start
            ...
            if (EFI_ERROR(Status)) { 
                ZeroMemory (&TempSataDevInterface, sizeof(SATA_DEVICE_INTERFACE));

                TempSataDevInterface.PortNumber = PortNumber;
                TempSataDevInterface.PMPortNumber = 0xFF;
                TempSataDevInterface.AhciBusInterface = AhciBusInterface;
                TempSataDevInterface.DeviceState = DEVICE_IN_RESET_STATE;

                // Update Base addresses
                TempSataDevInterface.PortCommandListBaseAddr = AhciBusInterface->PortCommandListBaseAddr;
                
                // HBA_PORTS_FB will have MappedFis Address, so finding the allocated memory of Fis address.
                TempSataDevInterface.PortFISMappedBaseAddr = 
                        HBA_PORT_REG64 (AhciBusInterface, PortNumber, HBA_PORTS_FB);
                TempSataDevInterface.PortFISBaseAddr = AhciBusInterface->PortFISBaseAddr + 
                        (TempSataDevInterface.PortFISMappedBaseAddr - AhciBusInterface->PortFISMappedBaseAddr);
...
)
{
    ...
    if (!SataDevInterface){
        ...
        // Update Base addresses
        SataDevInterface->PortCommandListBaseAddr = AhciBusInterface->PortCommandListBaseAddr;
        
        // HBA_PORTS_FB will have MappedFis Address, so finding the allocated memory of Fis address.
        SataDevInterface->PortFISMappedBaseAddr = HBA_PORT_REG64 (AhciBusInterface, Port, HBA_PORTS_FB);
        SataDevInterface->PortFISBaseAddr = (AhciBusInterface->PortFISBaseAddr + \
		                                    (SataDevInterface->PortFISMappedBaseAddr - AhciBusInterface->PortFISMappedBaseAddr));
...
)
{
    ...
    if (HBA_REG32 (AhciBusInterface,  HBA_CAP) & HBA_CAP_SSS) {
        ...
        if ((HBA_PORT_REG32 (AhciBusInterface, Port, HBA_PORTS_SSTS) 
            ...
            Status = HandlePortComReset(AhciBusInterface, SataDevInterface, Port, 0xFF);
...
)
{
    ...
    if ((SataDevInterface->DeviceType == ATAPI) &&
       ...
       ((SataDevInterface->IdentifyData.General_Config_0 & 0x1f00) != (OPTICAL_MEMORY_DEVICE << 8))) {
        DEBUG((DEBUG_ERROR, "AHCI : Unsupported ATAPI device found,device type :%x\n", 
                (SataDevInterface->IdentifyData.General_Config_0 & 0x1f00) >> 8 ));
        return EFI_UNSUPPORTED;
    }
...
)
{
    ...
    SataDevInterface->DevicePathProtocol = DPAddNode(TempDevicePath, (EFI_DEVICE_PATH_PROTOCOL *)&NewDevicePath);
    return Status;
        ...
        for (dlink = AhciBusInterface->SataDeviceList.pHead; dlink; dlink = dlink->pNext) {
            ...
            if (!EFI_ERROR(Status)) {
                ...
                if(IsTcgDriverHandlesBlockSid()) {
                    ...
                    DEBUG((DEBUG_INFO,"\n AhciBus: Issue BlockSid  Status %r ",Status));
                }
            }
        }
     }
...
)
{
    ...
    DEBUG ((DEBUG_BLKIO,  " Port FIS Aligned Base Address      : %08lX\n",  AhciBusInterface->PortFISBaseAddr));
    DEBUG ((DEBUG_BLKIO,  " Port FIS Base Address End          : %08lX\n",  AhciBusInterface->PortFISBaseAddrEnd));
    DEBUG ((DEBUG_BLKIO,  " Port Command Aligned Base Address  : %08lX\n",  AhciBusInterface->PortCommandListBaseAddr));
    ...
    DEBUG ((DEBUG_BLKIO,  " Port CommandList Mapped BaseAddr   : %08lX\n",  AhciBusInterface->PortCommandListMappedBaseAddr));
    DEBUG ((DEBUG_BLKIO,  " Port CommandTable Mapped BaseAddr  : %08X\n",   AhciBusInterface->PortCommandTableMappedBaseAddr));
    DEBUG ((DEBUG_BLKIO,  " Port FIS Mapped BaseAddr           : %08X\n",   AhciBusInterface->PortFISMappedBaseAddr));
#endif
    return;
}
