 ...
 )
{
    ...
    for (; TotalNumberofBlocks >  0; TotalNumberofBlocks -= TransferLength) {  
        ...
        CommandStructure.AtapiCmd.Ahci_Atapi_Command[1] = (UINT8)LShiftU64(AtapiDevice->Lun, 5);
        CommandStructure.AtapiCmd.Ahci_Atapi_Command[2] = (UINT8)RShiftU64(((UINT32) LBA),  24);
        CommandStructure.AtapiCmd.Ahci_Atapi_Command[3] = (UINT8)RShiftU64(((UINT32) LBA), 16);
        CommandStructure.AtapiCmd.Ahci_Atapi_Command[4] = (UINT8)RShiftU64(((UINT16) LBA), 8);
        CommandStructure.AtapiCmd.Ahci_Atapi_Command[5] = (UINT8)(((UINT8) LBA) & 0xff);
        CommandStructure.AtapiCmd.Ahci_Atapi_Command[7] = (UINT8)RShiftU64(TransferLength, 8);        // MSB
    ...
    if((SataDevInterface->IdentifyData.Reserved_104_126[2] & BIT14) && // WORD 106 valid? - BIT 14 - 1
       ...
       (SataDevInterface->IdentifyData.Reserved_104_126[2] & BIT12)) { // WORD 106 bit 12 - Sectorsize > 256 words
        ...
        SectorSize = (UINT32)(SataDevInterface->IdentifyData.Reserved_104_126[13] + \
                              LShiftU64(SataDevInterface->IdentifyData.Reserved_104_126[14], 16)) * 2;
    }
...
)
{
    ...
    BuildCommandList(SataDevInterface, CommandList, AhciBusInterface->PortCommandTableMappedBaseAddr);
        ...
        *Data = 0;
        if (!EFI_ERROR(Status)) {
            *Data = PortFISBaseAddr->Ahci_Rfis[12] |    
                    (UINT32)LShiftU64(PortFISBaseAddr->Ahci_Rfis[4], 8) |  
                    (UINT32)LShiftU64(PortFISBaseAddr->Ahci_Rfis[5], 16) |     
                    (UINT32)LShiftU64(PortFISBaseAddr->Ahci_Rfis[6],  24);  
        }
    }
...
)
{
    ...
    if (Status == EFI_SUCCESS) {
        if(ReadCapacity == TRUE) {
            BlkMedia->LastBlock = ((EFI_LBA)LShiftU64(InputData[0], 24)) | ((EFI_LBA)LShiftU64(InputData[1], 16)) | ((EFI_LBA)LShiftU64(InputData[2], 8)) | InputData[3];
            BlkMedia->MediaPresent = TRUE;
            BlkMedia->MediaId ++;
            BlkMedia->BlockSize = ((UINT32)LShiftU64(InputData[4], 24)) | ((UINT32)LShiftU64(InputData[5], 16)) | ((UINT32)LShiftU64(InputData[6], 8)) | InputData[7];
            AtapiDevice->BlockSize = (UINT16)BlkMedia->BlockSize;
            ...
            BlkMedia->LastBlock = ((EFI_LBA)LShiftU64(InputData[4], 24)) | ((EFI_LBA)LShiftU64(InputData[5], 16)) | ((EFI_LBA)LShiftU64(InputData[6], 8)) | InputData[7];
            BlkMedia->MediaPresent = TRUE;
            BlkMedia->MediaId ++;
            BlkMedia->BlockSize = ((UINT32)LShiftU64(InputData[9], 16)) | ((UINT32)LShiftU64(InputData[10], 8)) | InputData[11];
            BlkMedia->ReadOnly = FALSE;
            AtapiDevice->BlockSize = (UINT16)(BlkMedia->BlockSize);
        }
        ...
        if (AtapiDevice->DeviceType != CDROM_DEVICE) {
            ...
            CommandStructure.ByteCount= ByteCount;
            Status = ExecutePacketCommand(SataDevInterface, &CommandStructure, 0);
            if ((Status == EFI_SUCCESS) && (CommandStructure.ByteCount > 8)) {
...
)
{
    ...
    if (!EFI_ERROR(Status)) {
        ...
        *OddType=(UINT16 )((LShiftU64(ProfileData[sizeof(GET_CONFIGURATION_HEADER)+4], 8)) + ProfileData[sizeof(GET_CONFIGURATION_HEADER)+5]);
    }
...
)
{
    ...
    PrintCommandFISAndCommandTableAddress ( CommandStructure, CommandList, Commandtable); 
    return EFI_SUCCESS;
}
...
) 
{
    ...
    EFI_STATUS    Status;
    UINT64        PortFISBaseAddr = SataDevInterface->PortFISMappedBaseAddr;
    UINT64        CommandListBaseAddress = AhciBusInterface->PortCommandListMappedBaseAddr;
    UINT32        Data32;
...
)
{
    ...
    UINT8                   Port = SataDevInterface->PortNumber;
    UINT8                   PMPort;
    SATA_DEVICE_INTERFACE   *SataPMDevInterface;
    SATA_DEVICE_INTERFACE   *SataPMPortDevInterface;
    UINT32                  Data32;
    UINT32                  Init_SStatus = 0;
...
{
    EFI_STATUS  Status;
    volatile    AHCI_RECEIVED_FIS  *FISAddress =  NULL;
    UINT32      Data32;

    DEBUG ((DEBUG_INFO, "AHCI: PortReset on Port : %x PMPort : %x", Port, PMPort));
    if((AhciBusInterface == NULL) || (SataDevInterface == NULL)) {
        return EFI_INVALID_PARAMETER;
    }
    FISAddress = (AHCI_RECEIVED_FIS *)SataDevInterface->PortFISBaseAddr;
    if (!FISAddress) return EFI_DEVICE_ERROR;   // FIS receive address is not programmed.

    if (gPortReset) return EFI_DEVICE_ERROR;
    gPortReset = TRUE;
    ...
    if (PMPort == 0xFF) {
       ...
       HBA_PORT_REG32_AND_OR (AhciBusInterface, Port, HBA_PORTS_SCTL, 0xFFFFF000, 
                    HBA_PORTS_SCTL_DET_INIT + LShiftU64(Speed, 4) + LShiftU64(PowerManagement, 8));     
        pBS->Stall (1000);                               // 1msec
        HBA_PORT_REG32_AND (AhciBusInterface, Port, HBA_PORTS_SCTL, ~HBA_PORTS_SCTL_DET_MASK);
    } else {
        Data32 = HBA_PORTS_SCTL_DET_INIT + (UINT32)(LShiftU64(Speed, 4) + LShiftU64(PowerManagement, 8));
        ReadWritePMPort (SataDevInterface, PMPort, PSCR_2_SCONTROL, &Data32, TRUE);
        pBS->Stall (1000);                               // 1msec
        Data32 = (UINT32)(LShiftU64(Speed, 4) + LShiftU64(PowerManagement, 8));
        ReadWritePMPort (SataDevInterface, PMPort, PSCR_2_SCONTROL, &Data32, TRUE);
    }
    ...
    SataDevInterface->SControl = (UINT32)(LShiftU64(Speed, 4) + LShiftU64(PowerManagement, 8));

    DEBUG ((DEBUG_INFO, "AHCI : GeneratePortReset Status at Port : %x, PMPort : %x  :%r", Port, PMPort, Status));
	
    if (!EFI_ERROR(Status)) {
        // If Port Reset is success, then send SetMultipleMode command
        // Get latest IdentifyData to know whether SetMultipleMode command 
        // needs to be issued or not.
        Status = IssueSetMultipleModeCommand(SataDevInterface, TRUE);
        DEBUG ((DEBUG_INFO, "AHCI: IssueSetMultipleModeCommand : %r\n", Status));
    }
...
)
{
    ...
    if (DeviceDetected) {
        ...
        if ( i >= ATAPI_BUSY_CLEAR_TIMEOUT) {
		   DEBUG((DEBUG_INFO," AHCI: Device detection failed in PortComReset as PhyRdy bit was not set.\n"));
           return EFI_DEVICE_ERROR;
        }
        FISAddress = (AHCI_RECEIVED_FIS*)SataDevInterface->PortFISBaseAddr;
        ...
        if ( i >= ATAPI_BUSY_CLEAR_TIMEOUT) {
           DEBUG((DEBUG_INFO," AHCI: Device detection failed in PortComReset since D2H FIS was not received.\n"));
           return EFI_DEVICE_ERROR;
        }
        ...
        if (Data32 & (BIT30 + BIT29 + BIT28 + BIT27 + BIT26)){
            Status = EFI_DEVICE_ERROR;
            DEBUG ((DEBUG_ERROR, "AHCI: Port %x Interrupt Status :%x \n", Port,Data32));
        }
        ...
        // Check if valid signature is updated from the received D2H FIS.
        ...
        if (Data32 != ATA_SIGNATURE_32 && Data32 != ATAPI_SIGNATURE_32 && Data32 != PMPORT_SIGNATURE) {
            DEBUG ((DEBUG_ERROR, "AHCI HandlePortComReset:Invalid ATA/ATAPI/PM port signature, value: %x\n", Data32));
            return EFI_DEVICE_ERROR;
        }
...
)
{
    ...
    Data32 = HBA_PORT_REG32(AhciBusInterface, Port, HBA_PORTS_SIG);
    if (Data32 != ATA_SIGNATURE_32 && Data32 != ATAPI_SIGNATURE_32 && Data32 != PMPORT_SIGNATURE) {
        DEBUG ((DEBUG_ERROR, " AHCI : Invalid ATA/ATAPI/PM port signature, value: %x\n", Data32));
        return EFI_DEVICE_ERROR;
    }
...
VOID
PrintCommandFISAndCommandTableAddress (
  IN COMMAND_STRUCTURE    CommandStructure,
  IN AHCI_COMMAND_LIST        *CommandList,
  IN AHCI_COMMAND_TABLE       *Commandtable
) 
{
    ...
    DEBUG((DEBUG_BLKIO, "\n CommandList Base                         : %08X\n", CommandList));
    DEBUG((DEBUG_BLKIO, " CommandTable Mapped Base Address           : %08X\n", CommandList->Ahci_Cmd_CTBA));
    DEBUG((DEBUG_BLKIO, " CommandTable Upper Mapped Base Address     : %08X\n", CommandList->Ahci_Cmd_CTBAU));
    DEBUG((DEBUG_BLKIO, "\n CommandTable Base                        : %08X\n", Commandtable));
#endif
    return ;
}
