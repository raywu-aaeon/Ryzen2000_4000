...
)
{
    ...
    for (LoopCount  = 0; LoopCount < 1000; LoopCount++) {           // 10sec loop ( 1000 * 10 msec = 10Sec)
        ...
        if ((gAhciBusSmmProtocol->AtapiDevice.Atapi_Status != BECOMING_READY) && (gAhciBusSmmProtocol->AtapiDevice.Atapi_Status != MEDIA_CHANGED)){
            break;
        }
...
)
{
    ...
    Status = gAhciBusSmmProtocol->AhciSmmExecutePacketCommand( 
                                                        gAhciBusSmmProtocol,
                                                        &CommandStructure,
                                                        0,                      // Read
                                                        pDriveInfo->PortNum,
...
)
{
    ...
    // Validate if CommBuffer address falls in non-SMRAM region to avoid SMRAM data 
    ...
    if (AhciInt13SmmData->DriveCount == 0 || AhciInt13SmmData->DriveCount > SATA_PORT_COUNT) {
        return EFI_SUCCESS;
    }
    ...
    // UnRegister this SMI handler as SATA drive information for Int13 processing was saved.
    pSmstPi->SmiHandlerUnRegister (DispatchHandle);
    return EFI_SUCCESS;
}
