    ...
    // driver to the data used by INT13
     for (Count=0, i=0; i<DeviceCount; i++, Devices = ((UINTN*)Devices + 1)) {
       ...
       if (EFI_ERROR (Status)) 
        ...
        //Under Raid mode, ATA devices are not added into BBS table and ATAPI devices should be added
       // only if SUPPORT_ATAPI_IN_RAID_MODE token is enabled.
       if((PciConfig [IDE_SUB_CLASS_CODE] != SCC_RAID_CONTROLLER) ||
           ((PciConfig [IDE_SUB_CLASS_CODE] == SCC_RAID_CONTROLLER) && (Device->DeviceType == ATAPI)
                   && SUPPORT_ATAPI_IN_RAID_MODE)){
