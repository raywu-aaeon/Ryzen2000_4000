...
struct _OHCI_HCCA_PTRS {
    UINT32      InterruptList[OHCI_FRAME_LIST_SIZE];
...
/**
 ...
 Fields:   Name       Type        Description
      ...
      CallBack   NEAR       Pointer to call back function
      ActiveFlag BYTE       Non-zero value indicates TD is active
      DeviceAddr BYTE       USB device address
...
struct _OHCI_TD {
    UINT32      ControlStatus;         // Control and status fields
    UINT32      CurrentBufferPointer; // Current buffer pointer
    UINT32      LinkPointer;          // Pointer to the next TD
    UINT32      BufferEnd;            // End pointer of data buffer

    UINT32      CSReloadValue;         // Control status reload value
    UINT8       CallBackIndex;         // Index of the routine to call on completion
    UINT8       ActiveFlag;            // Routine to call on completion
    UINT8       DeviceAddr;            // Device Address
    UINT8       Resvd;
    UINT8       SetupData[8];          // Used for setup packet
};
...
struct _OHCI_ED {
    UINT32      Control;               //  ED control fields
    UINT32      TailPointer;          // TD queue tail pointer
    UINT32      HeadPointer;          // TD queue head pointer
    UINT32      EDLinkPointer;    // Pointer to next ED
    UINT8       Interval;
    UINT8       Reserved[15];
};
...
// Do not change the following order of EDRepeat and TDRepeat
    ...
    OHCI_ED     *EDControl;
    OHCI_TD     *TDControlSetup;
    OHCI_TD     *TDControlData;
    OHCI_TD     *TDControlStatus;

    OHCI_ED     *EDInterrupt;
    OHCI_TD     *TDInterruptData;
...
#define     GTD_NOT_ACCESSED_32     0xF0000000
