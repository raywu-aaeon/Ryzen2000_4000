...
typedef struct _OHCI_TD_
{
    ...
    UINT8 *CurrentBufferPointer;      // Current buffer pointer
    UINT8 *LinkPointer;               // Pointer to the next TD
    UINT8 *BufferEnd;                 // End pointer of data buffer
        #define MAX_CONTROL_DATA_SIZE  0x200
        
    UINT32 CSReloadValue;              // Control status reload value
    UINT8 CallBackIndex;               // Index of the routine to
                                      // call
                                      // on completion
    UINT8 ActiveFlag;                // Routine to call on
                                      // completion
...
typedef struct _OHCI_ED_
{
    ...
    POHCI_TD TailPointer;                 // TD queue tail pointer
    POHCI_TD HeadPointer;                 // TD queue head pointer
    struct _OHCI_ED_ *EDLinkPointer;      // Pointer to next ED
    UINT8 Reserved[16];
} OHCI_ED, *POHCI_ED;
...
typedef struct _PEI_OHCI_DEV
{
    ...
    UINT8                           NumPorts;
    OHCI_DESC_PTRS                  OHCIDescPtrs;
    UINT16                          AsyncListSize;
    UINT32                          MaxBulkDataSize;
        #define MAX_OHCI_BULK_DATA_SIZE    4096
        
    UINT32                          *FrameList; // HC Communication Area base
