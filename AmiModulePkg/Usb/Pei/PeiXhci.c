    ...
    IN EFI_PEI_SERVICES    **PeiServices )
{
    ...
    for (ControllerIndex = 0; ControllerIndex < gUsbPeiData->XhciHcCount;
        ...
        Usb3Hc->UsbHostControllerPpi.Bdf = (UINT16)((gXhciControllerPciTable[ControllerIndex].Bus << 8) | (gXhciControllerPciTable[ControllerIndex].Device << 3) | gXhciControllerPciTable[ControllerIndex].Function);
...
)
{
    ...
    EpCtx = (XHCI_EP_CONTEXT*)XhciGetContextEntry(Usb3Hc, XhciGetDeviceContext(Usb3Hc, SlotId), Dci);
...
)
{
    ...
    XhciExecuteCommand(PeiServices, Usb3Hc, XhciTEvaluateContextCmd, &SlotId16);
...
)
{
    ...
    *(UINT16*)&((XHCI_SETUP_XFR_TRB*)Trb)->RequestType = Rq;
    ((XHCI_SETUP_XFR_TRB*)Trb)->Value = Request->Value;
    ((XHCI_SETUP_XFR_TRB*)Trb)->Index = Request->Index;
    ((XHCI_SETUP_XFR_TRB*)Trb)->Length = (UINT16)(*DataLength);
    ...
    // Data TRB
    if (*DataLength != 0) {
        DEBUG((DEBUG_INFO, "Pei xHCI DataBuffer MMIO Address %x \n",Data));
        Trb = XhciAdvanceEnqueuePtr(XfrRing);
...
)
{
    ...
    ZeroMem((VOID*)RingBase, RingSize*sizeof(XHCI_TRB)); // Clear allocated memory
...
// check route string here
    ...
    SlotId16 = ((UINT16)SlotId & 0x00FF);
    Status = XhciExecuteCommand(PeiServices, Usb3Hc, XhciTConfigureEndpointCmd, &SlotId16);
    //ASSERT(Status == EFI_SUCCESS);
