...
// The maximum size for the Admin Submission and Admin completion
// Queue is defined as 4K slots.This is a zero based value.
#define MAX_ADMIN_QUEUE_ENTRIES          4095

// Minimum memory Page size allowed by NVMe driver
#define MIN_MPS_ALLOWED                  EFI_PAGE_SIZE

// Number of Admin submission queue entries , which is zero based.
// The complete Admin submission queue size is 4KB. For 4KB MPS
// this value will be 63.
#define ADMIN_SUBMISSION_QUEUE_SIZE      ((MIN_MPS_ALLOWED / sizeof(NVME_ADMIN_COMMAND)) - 1)

// Number of Admin completion queue entries , which is zero based.
// The complete Admin completion queue size is 4KB. For 4KB MPS this
// value will be 255.
#define ADMIN_COMPLETION_QUEUE_SIZE      ((MIN_MPS_ALLOWED / sizeof(COMPLETION_QUEUE_ENTRY)) - 1)

// Currently NVMe driver is not having Non-Blocking Admin command support.
// So use the same queue size for both admin submission and completion queues.
#define ADMIN_QUEUE_SIZE                 MIN(ADMIN_SUBMISSION_QUEUE_SIZE, ADMIN_COMPLETION_QUEUE_SIZE)

// Number of I/O submission queue entries , which is zero based.
// The complete I/O submission queue size is 4KB.For 4KB MPS
// this value will be 63.
#define QUEUE1_SUBMISSION_QUEUE_SIZE     ((MIN_MPS_ALLOWED / sizeof(NVME_ADMIN_COMMAND)) - 1)

// Number of I/O completion queue entries , which is zero based.
// The complete I/O completion queue size is 4KB.For 4KB MPS this
// value will be 255.
#define QUEUE1_COMPLETION_QUEUE_SIZE     ((MIN_MPS_ALLOWED / sizeof(COMPLETION_QUEUE_ENTRY)) - 1)

// Currently NVMe driver is not having Non-Blocking I/O command support.
// So use the same queue size for both I/O submission and completion queues.
#define QUEUE1_SIZE                      MIN(QUEUE1_SUBMISSION_QUEUE_SIZE , QUEUE1_COMPLETION_QUEUE_SIZE)

#define MAX_VALID_ACTIVE_NSID       1024
...
typedef struct {
    ...
    UINTN                              PciDev;
    UINTN                              PciFunc;
    UINT64                             AdminSubmissionBuffer;
    UINT64                             AdminCompletionBuffer;
    UINT64                             IoSubmissionBuffer;
    UINT64                             IoCompletionBuffer;
}NVME_QUEUE_ADDRESS_INFO;
