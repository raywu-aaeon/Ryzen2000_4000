...
AMI_STATUS_CODE_BUFFER *StausCodeBuffer = NULL;
    ...
    )
{
    ...
    if ((RouterCallbackStr == NULL) || (InterlockedCompareExchange16 (&RouterRecurciveStatus, 0, 1) == 1)) 
    ...
    for (i = 0; i != RouterCallbackStr->RegisteredEntries; i++) 
    {
        ...
        if (CallbackEntry->EndPointer > (CallbackEntry->StatusCodeDataBuffer + (CallbackEntry->BufferSize / 8) * 7)) 
        {
            if ((CurrentTpl < TPL_HIGH_LEVEL) && !InRscHandlerNotification)
            {
                ...
                if (!EFI_ERROR(Status) && NewBuffer != NULL && CallbackEntry->StatusCodeDataBuffer != 0)
                {
...
VOID EFIAPI ScRuntimeVirtualAddressChange (IN EFI_EVENT Event, IN VOID *Context)
{
    ...
    pRS->ConvertPointer(0,(VOID**)&String);
...
)
{
    ...
    Status = pBS->AllocatePool (EfiRuntimeServicesData, 
                                        sizeof (AMI_STATUS_CODE_BUFFER), 
                                       (VOID**) &StausCodeBuffer); 
    ASSERT_EFI_ERROR (Status);
...
{    
    if (CheckpointEvent != NULL){
        ...
        pBS->SignalEvent(CheckpointEvent);
    }
    return;
}
...
)
{
    ...
    UnicodeSPrint(s, 10*sizeof(CHAR16), L"%X",CheckpointInt);
