...
)
{
    ...
    Status = pRS->ConvertPointer(0, (void**)&SmbiosTablePtr);
    ASSERT_EFI_ERROR(Status);
    SmbiosTableEntryPoint->TableAddress = (UINT32)(UINTN)SmbiosTablePtr;
    Status = pRS->ConvertPointer(0, (void**)&SmbiosTableEntryPoint);
    ASSERT_EFI_ERROR(Status);
#endif
#if (SMBIOS_3X_SUPPORT == 1)
    ...
    Status = pRS->ConvertPointer(0, (void**)&SmbiosV3TableEntryPoint);
    ASSERT_EFI_ERROR(Status);
#endif
#if (FLASH_BLOCK_VIRTUALIZATION == 1)
    Status = pRS->ConvertPointer(0, (void**)&gVirtFlashBlockPtr);
    ASSERT_EFI_ERROR(Status);
#endif
    Status = pRS->ConvertPointer(0, (void**)&ScratchBufferPtr);
    ASSERT_EFI_ERROR(Status);
}
...
)
{
    ...
    CommBuffer->NumberOfStructures = GetNumberOfStructures((UINT8*)(UINTN)SmbiosV3TableEntryPoint->TableAddress);
    CommBuffer->LargestStructureSize = GetLargestStructureSize((UINT8*)(UINTN)SmbiosV3TableEntryPoint->TableAddress);
    CommBuffer->DmiStorageBase = (UINT64)(UINTN)SmbiosV3TableEntryPoint->TableAddress;
    CommBuffer->DmiStorageSize = MaximumBufferSize;
