...
)
{
    ...
    if (AmiValidateMemoryBuffer((VOID*)(UINTN)CommBuffer->DmiBiosRevision, 1) == EFI_SUCCESS) {
        CommBuffer->DmiBiosRevision = (SMBIOS_2X_MAJOR_VERSION << 4) | SMBIOS_2X_MINOR_VERSION;
    }
    else {
        return DMI_BAD_PARAMETER;
    }
    if (AmiValidateMemoryBuffer((VOID*)(UINTN)CommBuffer->NumberOfStructures, 2) == EFI_SUCCESS) {
        CommBuffer->NumberOfStructures = GetNumberOfStructures((UINT8*)(UINTN)SmbiosTableEntryPoint->TableAddress);
    }
    else {
        return DMI_BAD_PARAMETER;
    }
    if (AmiValidateMemoryBuffer((VOID*)(UINTN)CommBuffer->LargestStructureSize, 2) == EFI_SUCCESS) {
        CommBuffer->LargestStructureSize = GetLargestStructureSize((UINT8*)(UINTN)SmbiosTableEntryPoint->TableAddress);
    }
    else {
        return DMI_BAD_PARAMETER;
    }
    if (AmiValidateMemoryBuffer((VOID*)(UINTN)CommBuffer->DmiStorageBase, 4) == EFI_SUCCESS) {
        CommBuffer->DmiStorageBase = SmbiosTableEntryPoint->TableAddress;
    }
    else {
        return DMI_BAD_PARAMETER;
    }
    if (AmiValidateMemoryBuffer((VOID*)(UINTN)CommBuffer->DmiStorageSize, 2) == EFI_SUCCESS) {
    ...
    if (AmiValidateMemoryBuffer((VOID*)(UINTN)CommBuffer->DmiStorageBase, 8) == EFI_SUCCESS) {
...
)
{
    ...
    if (AmiValidateMemoryBuffer((VOID*)&CommBuffer->Handle, 2) == EFI_SUCCESS) {
        SmbStructurePtr = GetStructureByHandleThenUpdateHandle((VOID*)&CommBuffer->Handle);
...
#if (SMBIOS_2X_SUPPORT == 1)
    if (!SmbiosTableEntryPoint) return DMI_FUNCTION_NOT_SUPPORTED;
    MemCpy(BufferPtr, (UINT8*)(UINTN)SmbiosTableEntryPoint->TableAddress, MaximumBufferSize);
#else
    if (!SmbiosV3TableEntryPoint) return DMI_FUNCTION_NOT_SUPPORTED;
    MemCpy(BufferPtr, (UINT8*)(UINTN)SmbiosV3TableEntryPoint->TableAddress, MaximumBufferSize);
#endif                                          // SMBIOS_2X_SUPPORT
