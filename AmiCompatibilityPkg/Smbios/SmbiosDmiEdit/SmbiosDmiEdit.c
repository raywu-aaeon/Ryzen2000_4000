...
)
{
    ...
    AMI_SMM_COMM_BUFFER_X86_REGISTERS   AmiSmmCommRegister;
    
#if DMIEDIT_DEBUG_TRACE
    DEBUG((DEBUG_INFO, "In SmiHandler\n"));
#endif

    // This validation is to check whether common buffer is in SMRAM region. 
    ...
    Data = SwContext->CommandPort;
    
    if (IS_AMI_SMM_COMMUNICATION_X86_REG_SUPPORTED) {
...
#elif (SMBIOS_3X_SUPPORT == 1)  
        if (SmbiosTableSize == 0 || (SmbiosTableSize != (UINTN)((SMBIOS_3X_TABLE_ENTRY_POINT*)SmbiosV3TableEntryPoint)->TableMaximumSize)) {   
            // Call GetRemainingStructuresSize with NOT_IN_SMBIOS_TABLE since
            // SmbiosTableSize is not known.
            SmbiosTableSize = GetRemainingStructuresSize(SmbiosTable, NOT_IN_SMBIOS_TABLE);
        }
    ...
    if (IS_AMI_SMM_COMMUNICATION_X86_REG_SUPPORTED){
        AmiSmmCommunication->GetRegisterBlock (
                                            AmiSmmCommunication,
                                            Data,
                                            &AmiSmmCommRegister);
                
        pInterface = (VOID*)AmiSmmCommRegister.Rbx;
#if (SMBIOS_2X_SUPPORT == 0) && (SMBIOS_3X_SUPPORT == 1)
        {
            VOID             *pInterfaceHigh;
            pInterfaceHigh = (VOID *)AmiSmmCommRegister.Rcx;
            pInterface = (VOID*)((UINT64)pInterface | Shl64((UINT64)pInterfaceHigh, 32));
        }
#endif
        DEBUG((DEBUG_INFO, " ReadSaveState is success using SmmCommunication \n"));
    } else {
...
#endif
        DEBUG((DEBUG_INFO, "ReadSaveState is success using SmmCpuProtocol \n"));
    }
            ...
            if (AmiValidateMemoryBuffer((VOID*)pInterface, Size) == EFI_SUCCESS) {
                ...
                switch(Data) {
                    ...
                    case 0x52:
                                ...
                                if ((UINTN)SmbiosTable < (UINTN)0xf0000) {  
                                    EnableShadowWrite();
                                }
...
SmiHandler_Exit:
    if (IS_AMI_SMM_COMMUNICATION_X86_REG_SUPPORTED) {
        ...
        //ZeroMem(&AmiSmmCommRegister, sizeof(AmiSmmCommRegister));
        AmiSmmCommRegister.Rax = RetStatus;
        AmiSmmCommunication->SetRegisterBlock (
                                     AmiSmmCommunication,
                                     Data,
                                     &AmiSmmCommRegister);
        DEBUG((DEBUG_INFO, "WriteSaveState is success using SmmCommunication\n"));
        ...
        SmmCpuProtocol->WriteSaveState(
                                ...
                                Cpu,
                                &RetStatus);
        DEBUG((DEBUG_INFO, "WriteSaveState is success using SmmCpuProtocol \n"));
    }
