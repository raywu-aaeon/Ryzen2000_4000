...
WriteOnceStatusInit(VOID)
{
    ...
    if (Status == EFI_NOT_FOUND) {
        ...
        Status = pRS->SetVariable(L"WriteOnceStatus",
                        &gAmiSmbiosNvramGuid,
...
#endif                                          // SMBIOS_3X_SUPPORT
#endif                                          // SMBIOS_2X_SUPPORT
    if (AvailableBlkSize < (AsciiStrLen((char*)Data) + 1)) {
...
)
{
    ...
    if (AvailableBlkSize < (AsciiStrLen((CHAR8*)Data) + 1)) {
        return DMI_BAD_PARAMETER;               // Not enough space
    }
    ...
    // string fields
    for (i = 1; i < (Count + 1); i++) {
        StringSize = (UINT16)AsciiStrLen((CHAR8*)TablePtr) + 1;  // Size including string NULL terminator
        if (TableInfo.Offset == i) {
            BlockSize = (UINT16)AsciiStrLen((CHAR8*)Data) + 1;
...
)
{
    ...
    if (Status == EFI_NOT_FOUND || Count == 1) {
