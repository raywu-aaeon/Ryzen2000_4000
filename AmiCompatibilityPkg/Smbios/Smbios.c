...
/**
    Updates Smbios handles dynamically and sets global variable
...
)
{
    ...
    TempPtr = StructPtr + AsciiStrLen((char*)StructPtr) + 1;    // Move pointer to next string
...
)
{
    ...
    if (TestFlag == 0){
        ...
        for (i = 0; i < Count; i++) {
            StringSize = (UINT16)(AsciiStrLen((char*)SrcBuffer) + 1);       // Size including string NULL terminator
            if (NvramData->Flag[i]) {
                NvramStringSize = (UINT16)(AsciiStrLen(NvramData->StringSet[i]) + 1);
...
/**
    Reads SMBIOS Table from FV_SMBIOS

    @param StaticTableFfsGuid
    @param TableDataPtr
    @param TableSize
...
**/
EFI_STATUS
ReadTableFromFvSmbios (
    EFI_GUID    *StaticTableFfsGuid,
    VOID        **TableDataPtr,
    UINTN       *TableSize
)
{
    ...
    for (RomLayout = AmiGetFirstRomArea(); 
        ...
        // Get Table size from DMI Data Table Header
        *TableSize = (UINTN)*(UINT32*)((UINTN)TablePtr + 4);
        ...
        pBS->CopyMem(*TableDataPtr, TablePtr, *TableSize);
...
)
{
    ...
    Status = ReadTableFromFvSmbios(&FlashDataFile, &FlashData, &FlashDataSize);
    if (EFI_ERROR(Status)) goto Function_Exit;

    FlashDataPtr = FlashData;
    FlashDataEnd = FlashData + FlashDataSize;
    ...
    if (Status == EFI_SUCCESS) {
        ...
        for (Index = 0; Index < Count; Index++) {
            ...
            if (Status == EFI_NOT_FOUND) {
                ...
                //For Backward compatibility, covert formated string to Lower case 
                //and search the Variable
                CHAR16                  TempVar[17];
                UINT8                   TempIndex = 0;
                
                UnicodeSPrint (TempVar, 
                               sizeof (TempVar), 
                               L"%02x%04x%02x%02x",
                               Type, 
                               Handle, 
                ...
                while ((TempVar [TempIndex] != L'\0') && (TempIndex < (sizeof(TempVar) / sizeof(CHAR16)))) {
                    if (TempVar [TempIndex] >= L'A' && TempVar [TempIndex] <= L'F') {
                        TempVar [TempIndex] = TempVar [TempIndex] + (L'a' - L'A');
                    }
                    TempIndex++;
                }
...
)
{
    ...
    if (EFI_ERROR(Status) && Status != EFI_NOT_FOUND) {
        ...
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get SMBIOS_BASEBOARD_DATA\n"));
        return Status;
    }
...
)
{
    ...
    if (EFI_ERROR(Status) && Status != EFI_NOT_FOUND) {
        ...
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get SMBIOS_CHASSIS_DATA \n"));
        return Status;
    }
...
)
{
    ...
    if (EFI_ERROR(Status)) {
        ...
        if (EFI_ERROR(Status)) {
            ...
            DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get SMBIOS_CPU_DATA \n"));
            return Status;
        }
    }
    ...
    for (i = 0; i < gSmbiosBoardProtocol->NumberOfCPU; i++) {
      SrcBuffer = Buffer;
      if (FindStructureType(&SrcBuffer, &StructureFoundPtr, 4, i + 1)) {
...
)
{
    ...
    if (EFI_ERROR(Status) && Status != EFI_NOT_FOUND) {
        ...
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get SMBIOS_SLOTS_DATA \n"));
        return Status;
    }
...
)
{
    ...
    if (EFI_ERROR(Status) && Status != EFI_NOT_FOUND) {
        ...
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get SMBIOS_ONBOARD_DEV_DATA \n"));
        return Status;
    }
...
)
{
    ...
    if (AsciiStrLen((char*)StringData) == 0) {
...
)
{
    ...
    if (IdTblPtr->VendorId != 0xff) {
        ...
        Index = 0;
        Status = EFI_NOT_FOUND;
        while (AsciiStrCmp(ManufacturerTable[Index].Manufacturer, "Undefined")) {
            ...
            for (j = 0; j < gSmbiosBoardProtocol->NumberOfMemorySlots[i]; j++)
            {
                ...
                if (FindStructureType(&SrcBuffer, &StructureFoundPtr, 17, 1)){
                    ...
                    if (SmbDevice->Size == 0) {
                        ...
                        SmbDevice->ExtSpeed = 0;
                        SmbDevice->ExtConfigMemSpeed = 0;
#endif
                    }
                    else {
#if AMI_SMBIOS_DBG_MSG_SUPPORT == 1
...
#if SMBIOS_SPEC_VERSION > 320
                        if(DynDevice->Type17.Speed == 0xFFFF) {
                        SmbDevice->ExtSpeed = DynDevice->Type17.ExtSpeed;
                        }
                        else {
                            SmbDevice->ExtSpeed = 0;
                        }
                        if(DynDevice->Type17.ConfMemClkSpeed == 0xFFFF) {
                        SmbDevice->ExtConfigMemSpeed = DynDevice->Type17.ExtConfigMemSpeed;
                        }
                        else {
                            SmbDevice->ExtConfigMemSpeed = 0;
                        }
...
DynamicUpdateFlashMemoryData(VOID)
{
    ...
    PartNumber = AsciiStrStr(Manufacturer, " ");
...
)
{
    ...
    if (EFI_ERROR(Status) && Status != EFI_NOT_FOUND) {
        ...
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get SMBIOS_BATTERY_DATA \n"));
        return Status;
    }
...
)
{
    ...
    if (FindStructureType(&SrcBuffer, &AdditionalInfoPtr, 40, 1)) {
        ...
        for (i = 0; i < NumAdditionalInfoEntries; i++) {
            SrcBuffer = Buffer;
            StructureType = (UINT8)(((SMBIOS_ADDITIONAL_INFO*)AdditionalInfoPtr)->AdditionalEntries[i].RefHandle / 16);
            Instance = (UINT8)(((SMBIOS_ADDITIONAL_INFO*)AdditionalInfoPtr)->AdditionalEntries[i].RefHandle % 16);
...
)
{
    ...
    Status = pRS->GetVariable(
                        SMBIOS_ONBOARD_DEV_EXTENDED_DATA, 
                        &gAmiSmbiosDynamicDataGuid,
                        &Attributes, 
                        &SmbiosVariableSize, 
                        SmbiosVariableBuffer);
    if (EFI_ERROR(Status) && Status != EFI_NOT_FOUND) {
        ...
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get SMBIOS_ONBOARD_DEV_EXTENDED_DATA \n"));
        return Status;
    }
...
#else
    CHAR8   LangLong[][16] = {  "en|US|iso8859-1",
                                "fr|FR|iso8859-1",
                                "es|ES|iso8859-1",
                                "de|DE|iso8859-1",
                                "ru|RU|iso8859-5",
...
//----------------------------------------------------------------------------
    ...
    for (i = 0; i < LangCounts; i++) {
        ...
        for (j = 0; j < sizeof(SupportLang)/LANGUAGESIZE; j++) {
            if (!AsciiStrCmp(&SupportLang[j][0], CodeString)) {
                StrNumber++;
            ...
            if (!AsciiStrCmp(CodeString, Lang)) {
...
)
{
    EFI_STATUS    Status;
    UINT8         *TempPtr = NULL;
    UINT8         *SmbiosWakeUpTypeFoundPtr = NULL;
    ...
    if (FindStructureType (&TempPtr, &SmbiosWakeUpTypeFoundPtr, SMBIOS_TYPE_SYSTEM_INFORMATION , 1)) {
        SmbiosWakeUpTypeFoundPtr += OFFSET_OF (SMBIOS_TABLE_TYPE1, WakeUpType);   //0x18;
        Status = SaveLockBox (&gSmbiosWakeupTypeUpdateGuid,
                              &SmbiosWakeUpTypeFoundPtr,
                              sizeof(SmbiosWakeUpTypeFoundPtr));
        if (Status == EFI_SUCCESS) {
            // This case will be entered on first time saving data to SMM lock box.
            // So enable the required lock box attributes.
            Status = SetLockBoxAttributes (&gSmbiosWakeupTypeUpdateGuid, LOCK_BOX_ATTRIBUTE_RESTORE_IN_S3_ONLY);
            if (EFI_ERROR (Status)) {
                DEBUG((DEBUG_INFO, "Set LockBox Attribute Failed with status %r \n", Status));
            }
        }
    }
#endif    
...
#if SMBIOS_2X_SUPPORT
    Status = pRS->SetVariable(L"SmbiosEntryPointTable",
                    &gAmiSmbiosNvramGuid,
    ...
    Status = pRS->SetVariable(L"SmbiosEntryPointTableF000",
                    &gAmiSmbiosNvramGuid,
                    EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                    sizeof(SMBIOS_TABLE_ENTRY_POINT*),
                    &pSmbiosTableEntryPointF000);
...
#if SMBIOS_3X_SUPPORT
    Status = pRS->SetVariable(L"SmbiosV3EntryPointTable",
                    &gAmiSmbiosNvramGuid,
                    EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
    ...
    Status = pRS->SetVariable(L"SmbiosScratchBuffer",
                    &gAmiSmbiosNvramGuid,
                    EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                    sizeof(UINT8*),
                    &ScratchBufferPtr);
    ...
    Status = pRS->SetVariable(L"MaximumTableSize",
                    &gAmiSmbiosNvramGuid,
                    EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                    sizeof(UINT16),
                    &MaximumTableSize);
    ...
    if (*StringNumber) {
        if (FindStructureHandle(&StrucPtr, *SmbiosHandle)) {
            ...
            if (ValidateStringNumber(StrucPtr, ((SMBIOS_STRUCTURE_HEADER*)StrucPtr)->Type, *StringNumber)) {
                ...
                Status = pBS->AllocatePool(EfiBootServicesData,
                                        BufferSize + (UINT16)AsciiStrLen(String) + 1,
                                        (void**)&BufferPtr);
                if (EFI_ERROR(Status)) {
                    DEBUG((DEBUG_ERROR | DEBUG_WARN, "Memory allocation failed. Exit\n"));
...
#if ( defined(CSM_SUPPORT) && (CSM_SUPPORT != 0) )
{
    ...
    Status = pRS->GetVariable (L"SecureBoot", &gEfiGlobalVariableGuid, NULL, &VarSize, &SecureData);
    if (EFI_ERROR (Status)) {
        SecureData = 0;
    }
}
...
Install16bitPnPSmbiosFunctions()
{
    ...
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_INFO, "LegacyBiosExtProtocol not found. Exit\n"));
        return Status;
    }
