...
// Signaling compilers that a certain function cannot have a stack protector
//
#ifndef NOSTACKPROTECTOR
  #if defined (__GNUC__) || defined (__clang__)
    ///
    /// Signal compiler that the function cannot have stack protector
    ///
    ...
    /// Signal compiler that the function cannot have stack protector
    ///
    #define NOSTACKPROTECTOR __declspec(safebuffers)
  #else
    ///
    /// Signal compiler that the function cannot have stack protector
    ///
...
//#endif
// This change is due to compilation issues with the definitions of STATIC_ASSERT 
// using the WinDDK compiler.  This is only available in compilers that support C11. 
// The older Microsoft compiler did not have support for static_assert.  
#if _MSC_VER >= 1600 || __cplusplus >= 201103L
  // C++11 and newer or Visual Studio 10 and newer
  #define STATIC_ASSERT static_assert
#elif defined(MDE_CPU_EBC) || defined(__cplusplus) || defined(_MSC_EXTENSIONS)
  // EBC compiler, older C++ compilers, or older Visual Studio compilers
  #define STATIC_ASSERT(Expression, Message)
#else
  #define STATIC_ASSERT _Static_assert
#endif
