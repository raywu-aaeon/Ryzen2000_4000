...
#include "AmdRasRvDxe.h"

/*----------------------------------------------------------------------------------------
 *                   D E F I N I T I O N S    A N D    M A C R O S
 *----------------------------------------------------------------------------------------
 */
#define FILECODE RAS_AMDRASRVDXE_AMDRASRVDXE_FILECODE
 ...
 * @param[in] umcchno           always = 0 in RV
...
/*********************************************************************************
 * Name: AmdRasDxeRvInit
 *
 * Description
 *   Entry point of the AMD RAS RV DXE driver
...
CollectCpuMap(VOID)
{
  ...
  for (SocketLoop = 0; SocketLoop < NumberOfSockets; SocketLoop++) {
    ...
    for (DieLoop = 0; DieLoop < NumberOfDies; DieLoop++) {
      ...
      ApobInstanceId = MAKE_APOB_INSTANCE_ID (SocketLoop, DieLoop);
      for (ComplexLoop = 0; ComplexLoop < NumberOfComplexes; ComplexLoop++) {
        Status = ApobGetPhysComplexNumber (ApobInstanceId, CCD_INDEX_NOT_USED, (UINT32) ComplexLoop, &PhysComplexNumber);
