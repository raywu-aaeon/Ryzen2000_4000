...
#include <Protocol/AmdRasApei2Protocol.h>
#include "AmdRasSspApeiDxe.h"

/*----------------------------------------------------------------------------------------
 *                   D E F I N I T I O N S    A N D    M A C R O S
 *----------------------------------------------------------------------------------------
 */
#define FILECODE RAS_AMDRASSSPAPEIDXE_AMDRASSSPAPEIDXE_FILECODE
...
/*********************************************************************************
 * Name: AmdRasSspApeiDxeInit
 *
 * Description
 *   Entry point of the AMD RAS SSP APEI DXE driver
  ...
  )
{
  ...
  AMD_RAS_APEI2_PROTOCOL    *AmdRasApei2Protocol;
  UINTN                 NumberOfEnabledProcessors;
  UINTN                 NumberOfLogicProcessors = 0;

  DEBUG((EFI_D_ERROR, "[RAS] SSP AGESA RAS APEI driver entry\n"));

  // Allocate Memory for the the AMD_RAS_APEI_PROTOCOL protocol.
  Status = gBS->AllocatePool (
                              EfiBootServicesData,              // IN EFI_MEMORY_TYPE PoolType
                              sizeof (AMD_RAS_APEI2_PROTOCOL),  // IN UINTN Size
                              &AmdRasApei2Protocol               // OUT VOID **Buffer
                              );
  if (EFI_ERROR (Status)) {
  ...
  AmdRasApei2Protocol->McaErrorAddrTranslate = &AmdMcaErrorAddrTranslate;
  AmdRasApei2Protocol->TranslateSysAddrToCS = &AmdTranslateSysAddrToCS;
  AmdRasApei2Protocol->AddBootErrorRecordEntry = &AddBertErrorRecord;
  AmdRasApei2Protocol->AddHestErrorSourceEntry = &AddHestErrorRecord;
  AmdRasApei2Protocol->SearchMcaError = &AmdSearchMcaError;

  Status = gBS->InstallProtocolInterface (
                                          &Handle,                  // IN OUT EFI_HANDLE
                                          &gAmdRasApei2ProtocolGuid,// IN EFI_GUID
                                          EFI_NATIVE_INTERFACE,     // IN EFI_INITERFACE_TYPE
                                          AmdRasApei2Protocol        // IN VOID* Interface
                                          );
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
  ...
  )
{
  ...
  UINTN                     TableKey;
  EFI_ACPI_6_2_BOOT_ERROR_RECORD_TABLE_HEADER   *BertTable;             // Local helper pointer
  EFI_ACPI_6_2_BOOT_ERROR_REGION_STRUCTURE      *NewBertErrRgnTable;
  EFI_ACPI_6_2_BOOT_ERROR_REGION_STRUCTURE      *OrgBertErrRgnTable;
  ...
  BertTable = (EFI_ACPI_6_2_BOOT_ERROR_RECORD_TABLE_HEADER*)Table;

  // Get pointer to original Boot Error Region Ptr
  OrgBertErrRgnTable = (EFI_ACPI_6_2_BOOT_ERROR_REGION_STRUCTURE*) BertTable->BootErrorRegion;
  ...
  )
{
  ...
  UINTN                     TableKey;
  UINT8                     *HestTableEnd;
  EFI_ACPI_6_2_HARDWARE_ERROR_SOURCE_TABLE_HEADER *OldHestTable;             // Local helper pointer
  EFI_ACPI_6_2_HARDWARE_ERROR_SOURCE_TABLE_HEADER *NewHestTable;
  ...
  Table = NULL;
  Status = SearchAcpiTable(AcpiTableProtocol, AcpiSdtProtocol, EFI_ACPI_6_2_HARDWARE_ERROR_SOURCE_TABLE_SIGNATURE, &Table, &TableKey);
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "[RAS]HEST table not found !!!\n"));
    return Status;
  }

  OldHestTable = (EFI_ACPI_6_2_HARDWARE_ERROR_SOURCE_TABLE_HEADER *)Table;
