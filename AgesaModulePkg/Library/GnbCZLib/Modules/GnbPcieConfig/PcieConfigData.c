...
// todo:_CPU hardcode #include  "cpuServices.h"
...
#define FILECODE LIBRARY_GNBCZLIB_MODULES_GNBPCIECONFIG_PCIECONFIGDATA_FILECODE
/*----------------------------------------------------------------------------------------
  ...
  )
{
  ...
  AGESA_TESTPOINT (TpPcieConfigurationInitEntry, NULL);
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieConfigurationInit Enter\n");
// todo:_CPU hardcode for (SocketId = 0; SocketId < GetPlatformNumberOfSockets (); SocketId++) {
  for (SocketId = 0; SocketId < ST_SOCKETS; SocketId++) {
// todo:_CPU hardcode     if (IsProcessorPresent (SocketId, StdHeader)) {
...
// todo:          GetNodeId (SocketId, Silicon->SiliconId, &Silicon->NodeId, StdHeader);
        Silicon->NodeId = 0;
...
// todo:_CPU hardcode       }
  ...
  AGESA_TESTPOINT (TpPcieConfigurationInitExit, NULL);
  return Status;
}
...
// todo:   GetSocketModuleOfNode (0, &Node0SocketId, &Node0SiliconId, StdHeader);
  Node0SocketId = 0;
  Node0SiliconId = 0;
      ...
      if (NumberOfPorts != 0) {
        ...
        for (Index = 0; Index < NumberOfPorts; ++Index) {
          ...
          if (EngineDescriptor->EngineData.EngineType == PciePortEngine) {
            ...
            if (DescriptorLoLane >= DefaultSbPortDescriptor.EngineData.StartLane && DescriptorHiLane <= DefaultSbPortDescriptor.EngineData.EndLane) {
