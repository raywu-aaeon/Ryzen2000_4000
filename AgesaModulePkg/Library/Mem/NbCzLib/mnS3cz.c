...
#define FILECODE LIBRARY_MEM_NBCZLIB_MNS3CZ_FILECODE
...
PCI_REG_DESCRIPTOR ROMDATA S3PciPreSelfRefDescriptorCZ[] = {
  ...
  {{0, _4_BYTE_STORAGE, 0},  FUNC_5, 0x170, 0xFFF88EDB, 0},
  {{0, _4_BYTE_STORAGE, 0},  FUNC_5, 0x170, 0xFFF88EDB, 0},
  ...
  )
{
  ...
  BOOLEAN DctPopulated[MAX_DCTS_PER_NODE_CZ];

  NBPtr = S3NBPtr->NBPtr;
  MemPtr = NBPtr->MemPtr;

  if (S3NBPtr->MemS3PspPlatformSecureBootEn (S3NBPtr->NBPtr) == NBPtr->MemRunningOnPsp (NBPtr)) {
  ...
  )
{
  ...
  if (HeapLocateBuffer (&LocateBufferPtr, ConfigPtr) == AGESA_SUCCESS) {
    ...
    if (NBPtr->Dct != Dct) {
      ...
      PciWrite8 (Address.AddressValue, Dct);
      NBPtr->Dct = Dct;
      IDS_HDT_CONSOLE (MEM_SETREG, "~W F%d_%03x = %x\n", Address.Address.Function, Address.Address.Register, Dct);
      Address.Address.Function = TempFunc;
    }

    Address.Address.Register = 0x98;
    ExtendOffset &= 0x3FF;
    PciWrite32 (Address.AddressValue, ExtendOffset);
  ...
  )
{
  ...
  if (HeapLocateBuffer (&LocateBufferPtr, ConfigPtr) == AGESA_SUCCESS) {
    ...
    PciWrite32 (Address.AddressValue, ValueWrite);
    Address.Address.Register = 0x98;
    PciWrite32 (Address.AddressValue, ExtendOffset);
    IDS_OPTION_HOOK (IDS_AFTER_DCT_PHY_ACCESS, NULL, ConfigPtr);
    IDS_HDT_CONSOLE (MEM_SETREG, "~W F2_9C_%x = %x\n", Address, *(UINT32 *)Value);
  }
}
...
{
  if (NBPtr->Dct != *(UINT8 *) Dct) {
    //
    // Set the DctCfgSel to new DCT
    //
    MemNCmnGetSetFieldCZ (NBPtr, 1, RegDctCfgSel, *(UINT8 *) Dct);
  }
  ...
  )
{
  ...
  case AccessWidth8:
  case AccessS3SaveWidth8:
    PciWrite8 (Address.AddressValue, *((UINT8 *) &MiscIndex));
    break;
  case AccessWidth16:
  case AccessS3SaveWidth16:
    PciWrite16 (Address.AddressValue, *((UINT16 *) &MiscIndex));
    break;
  case AccessWidth32:
  case AccessS3SaveWidth32:
    PciWrite32 (Address.AddressValue, *((UINT32 *) &MiscIndex));
    break;
  default:
    IDS_ERROR_TRAP;
  }
