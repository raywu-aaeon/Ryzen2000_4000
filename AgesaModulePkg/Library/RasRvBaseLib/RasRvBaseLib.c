...
#include <Library/RasRvBaseLib.h>
 ...
 *----------------------------------------------------------------------------------------
 */
#define FILECODE LIBRARY_RASRVBASELIB_RASRVBASELIB_FILECODE
 ...
 * since one channel per UMC on RV)
 * The input parameter umc_chan_num is fixed to 0 on RV
 *
 * @param[in] pkg_no            Socket ID (0..1)
 * @param[in] mpu_no            Die ID (0..3)
 * @param[in] umc_inst_num      UMC ID (0..1)
 * @param[in] umc_chan_num      always = 0 in RV
  ...
  )
{
  ...
  umc_chan_num = 0;   // umc_chan_num = 0 in RV
  ...
  rm1 = (temp >> 4) & 0xf; 
  rm2 = (temp >> 8) & 0xf;
  //[15:12] reserved in RV.chan = (temp >> 12) & 0xf;
...
/*------------------------------------------------------------------
 ...
 Limitations:
   - Does not support non-power-of-2 channels (not used on RV)
  ...
  )
{
  ...
  // BOZO: finish this, for now, we're just using CsInstanceID = channel number (correct on RV)
