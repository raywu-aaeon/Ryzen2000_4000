 ...
 *----------------------------------------------------------------------------------------
 */
#define FILECODE        LIBRARY_APCBSSPLIB_GETSETPARAMS_FILECODE
  ...
  )
{
  ...
  //
  while ((((UINT8 *) ApcbGroupHeader + sizeof (APCB_GROUP_HEADER) - 1) <= ApcbEnding) && ( ((UINT8 *) ApcbTypeHeader + ApcbTypeHeader->SizeOfType - 1) <= ApcbEnding)) {
    if (ApcbGroupHeader->GroupId == GroupId) {
      if ((ApcbTypeHeader->TypeId == TypeId)  && (ApcbTypeHeader->InstanceId == InstanceId)) {
        *TypeData = (VOID *) ((UINT8 *) ApcbTypeHeader + sizeof (APCB_TYPE_HEADER));
        *Size = (ApcbTypeHeader->SizeOfType - sizeof (APCB_TYPE_HEADER));
        IDS_HDT_CONSOLE_PSP_TRACE ("\t\tFound matched type id @0x%x!\n", ApcbTypeHeader);
        return EFI_SUCCESS;
      } else {
        ApcbTypeHeader = (APCB_TYPE_HEADER *) ((UINT8 *) ApcbTypeHeader + ApcbTypeHeader->SizeOfType);
        // Adjust alignment
        ...
        if ((UINT8 *) ApcbTypeHeader > ((UINT8 *) ApcbGroupHeader + ApcbGroupHeader->SizeOfGroup)) {
      ...
      ApcbTypeHeader = (APCB_TYPE_HEADER *) ((UINT8 *) ApcbGroupHeader + sizeof (APCB_GROUP_HEADER));
    }
  }
