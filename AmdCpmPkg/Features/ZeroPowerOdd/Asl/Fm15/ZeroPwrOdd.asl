    ...
    )
{
    ...
    External(\_SB.PCI0.SATA._ADR, MethodObj)
    ...
    Scope(\_SB.PCI0.SATA) {
            ...
            If (LEqual(XCMP(Arg0, Buffer(){0x9B, 0x14, 0xDB, 0xE4, 0xFE, 0xFC, 0x5B, 0x42, 0xA6, 0xD8, 0x92, 0x35, 0x7D, 0x78, 0xFC, 0x7F}), 1)) {
                ...
                //
                    ...
                    If (LOr(LEqual(Local0, Add(Multiply(Local4, 0x10000), 0x0000FFFF)), LEqual(Local0, Ones))) {
                        ...
                        If (LEqual(Local1, 0)) {
                            \_SB.PCI0.SATA.STPT(0)              // Disable PHY Port 1
                            Return(0)
            ...
            If (LEqual(And(Local0, 3), 3)) {
                ...
                If (LEqual(Local6, 0)) {
                    If (CpmReadGevent(Local3)) {
                        Store(0x1, \_SB.PCI0.SATA.SAPO)
                    } Else {
                        Store(0x0, \_SB.PCI0.SATA.SAPO)
                    }
        ...
        Method (TWAK, 1) {
            ...
            If (LEqual(And(Local0, 3), 3)) {
                ...
                If (LEqual(Local6, 0)) {
                    If (CpmReadGevent(Local3)) {
                        If (LAnd(LEqual(Arg0, 4), LEqual(\_SB.PCI0.SATA.SAPO, 0x0))) {
                            If (LNot(CpmIsWin8())) {
                ...
                // Source of ODD_PLUGIN# - 1: KBC GPIO/QEVENT Pin, 2: ECRAM GPIO/QEVENT Pin
                    ...
                    If (LEqual(\_SB.PCI0.LPC0.EC0.OKEC, 0x01)) {
                        If (CpmReadGpio(Add(Local3, ShiftLeft(Local6, 8)))) {
                            If (LAnd(LEqual(Arg0, 3), LEqual(\_SB.PCI0.SATA.SAPO, 0x0))) {
                                If (LNot(CpmIsWin8())) {
                                    Notify(\_SB.PCI0.SATA.ODDZ, 0x1)
                                    Notify(\_SB.PCI0.SATA.ODDZ, 0x81)
                                }
                            } ElseIf (LAnd(LEqual(Arg0, 4), LEqual(\_SB.PCI0.SATA.SAPO, 0x0))) {
                                If (LNot(CpmIsWin8())) {
                                    Notify(\_SB.PCI0.SATA.ODDZ, 0x1)
                                    Notify(\_SB.PCI0.SATA.ODDZ, 0x81)
                                }
        ...
        Method(XL10) {
            ...
            If (CpmReadGeventTriggerType(Local3)) {
                ...
                If (CpmReadGevent(Local3)) {
                    ...
                    If (CpmIsWin8()) {
                        Store(0, \_SB.PCI0.SATA.ODD8.CpmOddStatus)
                    } Else {
                ...
                If (LNot(CpmReadGevent(Local3))) {
                    ...
                    Sleep(40)
                    If (CpmIsWin8()) {
                        Store(1, \_SB.PCI0.SATA.ODD8.CpmOddStatus)
                    } Else {
    ...
    Scope(\_SB.PCI0.LPC0.EC0) {
        ...
        Method(XQ43, 0x0, Serialized) {
            ...
            If (LAnd(Local6, And(Not(Local7), 1))) {
                ...
                If (CpmIsWin8()) {
                    If (LEqual(\_SB.PCI0.SATA.ODD8.CpmOddStatus, 0)) {
                        Notify(\_SB.PCI0.SATA.ODD8, 0x02)
                        CpmSetPostCode(CpmTpZeroPowerOddRemoveQEvent_ODD_DA)
                    }
                } Else {
                    If (LEqual(\_SB.PCI0.SATA.ODDZ.CpmOddStatus, 0)) {
                        Notify(\_SB.PCI0.SATA.ODDZ, 0x80)
                        CpmSetPostCode(CpmTpZeroPowerOddRemoveQEvent_ODD_DA)
                    }
                }
