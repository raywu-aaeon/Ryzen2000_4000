  ...
  )
{
  ...
  UINT8                      Ccx0DownCoreNum;
  UINT8                      Ccx1DownCoreNum;
  UINT8                      RemainingCoreNum;
  ...
  if (AodMemBuffer->Command == SET_MEMORY_CLOCK) {
    ...
    if (AodMemBuffer->DownCore.DownCoreConfig.IsBitmap == 0) { // Enabeld core number
      if (AodMemBuffer->DownCore.ActiveCoreNum == 6) {
        gCbsConfig->CbsCmnCpuOcMode = 5;
        gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 7;  // SIX (3 + 3)
      } else if (AodMemBuffer->DownCore.ActiveCoreNum == 4) {
        gCbsConfig->CbsCmnCpuOcMode = 5;
        gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 5;  // FOUR (2 + 2)
      } else if (AodMemBuffer->DownCore.ActiveCoreNum == 2) {
        gCbsConfig->CbsCmnCpuOcMode = 5;
        gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 2;  // TWO (1 + 1)
      } else if (AodMemBuffer->DownCore.ActiveCoreNum == 1) {
        gCbsConfig->CbsCmnCpuOcMode = 5;
        gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 1;  // ONE (1 + 0)
        ...
        gCbsConfig->CbsCmnCpuGenDie0DowncoreBitMap = 0;
        gCbsConfig->CbsCmnCpuGenDie1DowncoreBitMap = 0;
        gCbsConfig->CbsCmnCpuGenDie2DowncoreBitMap = 0;
        gCbsConfig->CbsCmnCpuGenDie3DowncoreBitMap = 0;
      } else {
        // Should not be here
        return EFI_UNSUPPORTED;
      }
    } else {  // DownCoreBitmap is the disabled core bitmap not number
      if ((UINT8) AodMemBuffer->DownCore.DownCoreConfig.DieNum == 0) {
        gCbsConfig->CbsCmnCpuGenDie0DowncoreBitMap = (UINT8) AodMemBuffer->DownCore.DownCoreConfig.DownCoreBitmap;
      } else if ((UINT8) AodMemBuffer->DownCore.DownCoreConfig.DieNum == 1) {
        gCbsConfig->CbsCmnCpuGenDie1DowncoreBitMap = (UINT8) AodMemBuffer->DownCore.DownCoreConfig.DownCoreBitmap;
      } else if ((UINT8) AodMemBuffer->DownCore.DownCoreConfig.DieNum == 2) {
        gCbsConfig->CbsCmnCpuGenDie2DowncoreBitMap = (UINT8) AodMemBuffer->DownCore.DownCoreConfig.DownCoreBitmap;
      } else if ((UINT8) AodMemBuffer->DownCore.DownCoreConfig.DieNum == 3) {
        gCbsConfig->CbsCmnCpuGenDie3DowncoreBitMap = (UINT8) AodMemBuffer->DownCore.DownCoreConfig.DownCoreBitmap;
      } else {  // Currently only support up to 4 dies
        return EFI_UNSUPPORTED;
      }
      Ccx0DownCoreNum = (UINT8) HammingWeight ((UINT8) AodMemBuffer->DownCore.DownCoreConfig.DownCoreBitmap & 0x0F);
      Ccx1DownCoreNum = (UINT8) HammingWeight ((UINT8) AodMemBuffer->DownCore.DownCoreConfig.DownCoreBitmap & 0xF0);
      RemainingCoreNum = (UINT8) (gAodCpuBiosStruct->ActiveCoreNum - Ccx0DownCoreNum - Ccx1DownCoreNum);
      if (Ccx0DownCoreNum != Ccx1DownCoreNum) {  // N + 0 mode
        if (RemainingCoreNum == 1) {
          gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 1;   // ONE (1 + 0)
        } else if (RemainingCoreNum == 2) {
          gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 3;   // TWO (2 + 0)
        } else if (RemainingCoreNum == 3) {
          gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 4;   // THREE (3 + 0)
        } else if (RemainingCoreNum == 4) {
          gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 6;   // FOUR (4 + 0)
        } else {
          return EFI_UNSUPPORTED;
        }
      } else {  // N + N mode
        if (RemainingCoreNum == 2) {
          gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 2;  // TWO (1 + 1)
        } else if (RemainingCoreNum == 4) {
          gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 5;  // FOUR (2 + 2)
        } else if (RemainingCoreNum == 6) {
          gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 7;  // SIX (3 + 3)
        } else {
          return EFI_UNSUPPORTED;
        }
      }
      if (AodMemBuffer->DownCore.DownCoreConfig.DownCoreBitmap == 0xFF) {
        gCbsConfig->CbsCmnCpuOcMode = 0;
        gCbsConfig->CbsCmnCpuGenDowncoreCtrl = 0; // Change back to Auto mode when all cores are enabled
        gCbsConfig->CbsCmnCpuGenDie0DowncoreBitMap = 0;
        gCbsConfig->CbsCmnCpuGenDie1DowncoreBitMap = 0;
        gCbsConfig->CbsCmnCpuGenDie2DowncoreBitMap = 0;
        gCbsConfig->CbsCmnCpuGenDie3DowncoreBitMap = 0;
      } else {
        // Still need to set OC mode to manual
        gCbsConfig->CbsCmnCpuOcMode = 5;
      }
    }
